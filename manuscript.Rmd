...
note: |
    Unsubmitted draft created from the commit `r repro::current_hash()`

    
authornote: |
 The article is prepared for submission as a master's thesis in fulfillment of the requirements for the degree Master of Science (M.Sc.) at
...
The model predicts an inverse relationship between the switching probability and the overall number of sampled outcomes, since the stochastic processes tend to terminate faster, the higher the switching probabilities.
In other words, the higher the switching probability, the smaller the samples on which each mean comparison is based, resulting in a decreasing overall number of sampled outcomes.
Since the relative change in the switching probability decreases as $\psi$ approaches certainty, the relative change in the number of sampled outcomes decreases respectively.
Furthermore, because it determines the number of mean comparisons---and therefore stochastic processes---that must be carried out, any increase in the threshold leads to an increase in the number of sampled outcomes.
...
Figure\ \@ref(fig:choice-rates) displays the rates of choices that did not maximize the EV for each combination of the switching probabilities and thresholds.
The top panel of the figure shows the rates for choice problems where the low-rank outcome of the risky prospect has a low probability.
Respectively, for high switching probabilities and therefore short stochastic processes, the low-rank outcome is expected to occur less often over the course of a stochastic process than would be objectively warranted, leading to an inflation of the respective mean of the risky prospect.
Accordingly, if the safe prospect possesses the higher EV, the number of false responses increases with the switching probability.
However, if the risky prospect has the higher EV, the mean inflation causes the choices to be generally in line with EV maximization.
Note from the middle panel that this pattern reverses, if the high-rank outcome has a small probability.
...
##merge figure
ggarrange(alpha, value, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right") 
```

(ref:cpt-graphs) Estimated Graphical Shapes of the Weighting and Value Function for Different Simulation Parameters

```{r cpt-graphs, fig.cap="(ref:cpt-graphs)", eval = FALSE}
#prepare data
...
##merge figures
ggarrange(weight, value, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right") 
```
