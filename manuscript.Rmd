...
In the domain of decision making, such inputs provide information about the choice alternatives among which people can choose, the latter being hereafter treated as elements $c$ of the set $C$.
That is, each choice alternative $c \in C$ is assigned some information from the inputs and this information may be used by the mind to learn about the *properties* of the respective alternative.
...
Rather, with the choice between at least two *prospects* [@kahnemanProspectTheoryAnalysis1979, see below], it studies a case which is counterfactual in that it omits the many particularities of each decision, but retains in the form of prospects the fundamental properties that almost all choice alternatives are assumed to possess [@lopesThoughtsPsychologicalConcept1983].^[See @knightRiskUncertaintyProfit1921, Chapter 1, for a timeless discussion of the reasonableness of such abstractions.] 
These fundamental properties are considered to be the possible outcomes of a choice alternative---hereafter denoted by $x$---and the probabilities with which these outcomes occur following the choice of the alternative---hereafter $p_X(x)$.
Roughly speaking, then, all choice alternatives that can be fully described by their outcomes and probabilities are prospects (or: gambles), which are exclusively considered below. 
...
Now, let the exact axiomatic definition of a prospect be that of a discrete random variable's distribution.
...
The random variable $X$ is then a measurable function
$$
\begin{aligned}
...
An outcome's probability $p(x)$ is then provided by the probability measure
$$
P_X : \mathcal{F'} \to [0,1]
\tag{5}
$$
with 
$$
P_X(A' \in \mathcal{F'}) := P(X^{-1} \times A') = P(\{\omega: X(\omega) \in A'\})
\tag{6}
$$
...
Accordingly, each $c$ can itself be considered a set of outcome-probability pairs $(x_i, p(x_i)) \in \{(x_1, p(x_1)), ..., (x_n, p(x_n))\}$.

However, rarely in peoples' daily life, inputs take the form of explicit descriptions of all outcomes-probability pairs<!--(as in Example 1)-->.
...
More formally, let the sample of a prospect be a set of realizations of independent and identically distributed (i.i.d.) random variables.^[Note that this paper considers the counterfactual case where samples are random, although it has been emphasized that in the natural world samples are "virtually never random" [@fiedlerBewareSamplesCognitiveecological2000, p. 660]. To account for non-random sampling, e.g., in the form of sampling biases [e.g., @fiedlerBewareSamplesCognitiveecological2000] or dynamic information structures [e.g., @cohenEffectPerceivedPatterns2021; @plonskyRelianceSmallSamples2015], the i.i.d-assumption would need to be dropped, whi
ch is out of the scope of this paper.]
Accordingly, for the choice between $1, ..., k$ prospects, the inputs to DfE are generated by the following set of stochastic processes:
$$
S = \{X_{1t}, ... , X_{kt}\}
\; ,
\tag{6}
$$
with $t \in \{1, ..., N\}$ denoting a sequence of realizations of $X_c$ and $N$ denoting the size of the respective sequence. 
...
\tag{8}
$$ 
with subjective values $v(x_i)$ derived from a *value function* $v$ that may transform the objective outcomes, and decision weights $\pi_i$ derived from the objective probabilities' cumulative distribution function that may be transformed by a *probability weighting function* $w$.^[Note that CPT or other originally deterministic decision models may be amended by a stochastic choice function to account for the probabilistic nature of choices [see, e.g., @rieskampProbabilisticNaturePreferential2008].] 
