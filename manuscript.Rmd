...
In the domain of decision making, such inputs provide information about the *properties* of the choice alternatives among which people can choose.
Hereafter, let us treat these alternatives as elements $c$ of the set $C$.
...
Importantly, information about the choice alternatives can come in many formats.
...
In *decisions from experience* [DfE, @hertwigDecisionsExperienceEffect2004], inputs take the form of *samples* (see below).
In this latter case---perhaps, the one that more closely resembles decisions in people's daily life---the properties of the choice alternatives are not described and therefore not known with certainty, but they must be inferred from the consequences that occurred or were absent after the alternatives were chosen in similar decisions in the past.
Although these different formats may in principle carry the same information about given choice alternatives, behavioral decision research has yet produced a large body of papers indicating that, taking a quite general perspective, there are robust differences between the choices that are made in DfD and DfE.
That is, all other things being equal, empirical choice patterns across studies indicate that in DfD people choose *as if* a rare consequence of a choice alternative has more impact on the latter's evaluation than would be objectively warranted by the principle of mathematical expectation (see below); in turn, in DfE, people choose *as if* the same rare consequence has less impact on the evaluation than warranted [e.g., @barronSmallFeedbackbasedDecisions2003; @hertwigDecisionsExperienceEffect2004; @weberPredictingRiskSensitivity2004; @hauDescriptionexperienceGapRisky2008; @rakowBiasedSamplesNot2008; @ungemachAreProbabilitiesOverweighted2009; @camilleriWhenWhyRare2011; @kellenHowVariantAre2016; @erevAnomaliesForecastsDescriptive2017; @regenwetterConstructBehaviorGap2017; see also @wulffMetaanalyticReviewTwo2018, for a comprehensive meta-analytic review].

This paper uses the *description-experience gap* [@hertwigDescriptionexperienceGapRisky2009] as an entry point for a closer inquiry into the sampling process as an essential part of sample-based decision making.  
...
Now, more formally,^[Paragraphs with this beginning usually formalize the main ideas of the preceding paragraphs or elaborate on them by using mathematical expressions and concepts. Although important, the gist of this paper should be captured despite skipping these paragraphs.]  let the axiomatic definition of a prospect be that of a discrete random variable's distribution.
...
Accordingly one may adopt an as-if weighting terminology for the description of choices by stating that people decide *as if* they maximized the value in (10) and applied the inferred over- and underweighting pattern [@pachurWeightUncertainEvents2019].
Importantly, the as-if prefix indicates that it cannot be concluded from the inferred cumulative decision weights that the mind does indeed perform any of the computations associated with the weighting of outcomes in CPT; rather, it processes the inputs in a way that the outputs, i.e., the resulting choice patterns, approximate the solutions from the inferred CPT model [see, e.g., @griffithsRationalUseCognitive2015]. 
Below, CPT will therefore only be used as a model to describe choices between prospects and it will be discussed---and demonstrated in a simulation---how distinct strategies of generating and integrating samples in DfE can contribute to weighting patterns that are commonly inferred when modeling respective empirical choice data.
