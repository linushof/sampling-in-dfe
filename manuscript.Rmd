...
```{r fig-sampling-paradigm, fig.cap="(ref:sampling-paradigm)"}
...
A summary of the model predictions can be found in Table\ \@ref(tab:tab-predictions).
...
```{r fig-weighting-function, fig.cap="(ref:weighting-function)"}
...
Overall, the results of the simulation match the predictions from Table\ \@ref(tab:tab-predictions), however, with some exceptions concerning the estimates for the parameter $\delta$ of the weighting function. 
...
```{r chain-length, fig.cap="(ref:chain-length)", eval = FALSE}
choices_absolute %>%
  group_by(model, s, a) %>% 
...
```{r choice-rates, fig.cap="(ref:choice-rates)", eval = FALSE}
#prepare data
...
gamma <- cpt_absolute %>%
  ...
  geom_point() +
  geom_line() +
  scale_color_viridis_d(option = "viridis", begin = .3, end = .8) + 
  theme_minimal()
...
wf <- weights %>% 
  ...
  facet_wrap(~a~model, nrow = 2, labeller = labeller(a = as_labeller(label_theta, default = label_parsed))) + 
