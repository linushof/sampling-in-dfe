...
1. *Lack of formal modeling/unstated theoretical commitments:*
The authors offer a verbal description of their theoretical claim that sampling strategies are systematically linked to decision strategies, but do not provide a formal specification or computational implementation of their claim.
Moreover, although the proposed links between sampling and decision strategies are used to "explain" the (non)occurrence of an as-if underweighting of rare outcomes pattern, no formal and complete accounts of the potential choice-generating mechanisms---which must contain the proposed links---are provided.
...
Theoretical frameworks that do so, e.g., sequential sampling models [e.g., @markantModelingChoiceSearch2015], make themselves assumptions about the decision-making process that may constrain how the a proposed link between sampling and decision strategies can be specified.
Failing to explicitly spell out theoretical commitments and/or to formally model the decision-making process can later on 

2. *Vague consideration of hybrid models:*
In principal, the sampling and decision strategies, as proposed, are independent of each other and can be freely combined---i.e., each instance of a sampling strategy can be combined with each instance of a decision strategy. 
