...
Such inputs may be treated as a set $I$ and its elements $i \in I$, the latter being distinguishable from each other on the basis of the *properties* they possess [cf. @jostMathematicalConcepts2015].
Often, however, the properties used by the mind to control the outputs are not readily described, but proper descriptions must be inferred from incomplete or noisy information. 
...
The concept of the power set thereby illustrates in a plain way how different selection criteria can alter the consequential choice, provided the properties are not uniformly distributed among the alternatives. 
\

(ref:figure2) The power set of the set $I$ including the choice alternatives A, B, and C. Black (white) circles indicate that an alternative does (not) possess a property. Each column displays a possible subset that could result from assessing whether the alternatives posses a relevant property.   
