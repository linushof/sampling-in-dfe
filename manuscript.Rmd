...
Figure\ \@ref(fig:weighting-patterns) shows the means and the 95%-intervals of the posterior distributions of the curvature parameter $\gamma$ and elevation parameter $\delta$ as well as the resulting shape of the weighting function. 
Note first that because the relative frequencies were supplied as probability information, the parameter estimates for the threshold value $\theta = 1$ in combination with high switching probabilities have large posterior intervals.
The estimates substantiate the prediction that $\gamma$ takes values over $1$ and increases with the switching probability, resulting in an S-shaped weighting function. 
Because the images of the function equal the decision weights, the estimates show that high switching probabilities translate to an underweighting of rare outcomes in CPT.
The estimates of $\delta$ also tend to increase with increasing switching probabilities.
