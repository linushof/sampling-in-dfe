...
However, except for the choice rates, displayed in Figures\ \@ref(fig:choice-rates-roundwise) and \@ref(fig:choice-rates-summary), the results of both models are displayed in the same figures using a green and blue color-coding for the round-wise and the summary integration model, respectively.
...
First, it is inspected how the lengths of the sampling and accumulation processes---measured in the number of sampled outcomes---changes with varying combinations of the switching probability $\psi$ and threshold $\theta$.

#### Number of Sampled Outcomes

Figure\ \@ref(fig:chain-length) displays the number of outcomes sampled over the course of a sampling and accumulation process for each parameter combination.
The model predicts a decrease in the overall number of sampled outcomes with increases in the switching probability $\psi$.
As can be seen from the changes in the slopes of the gray lines of the smoothed functions, the magnitude of this decrease diminishes as the relative increase in $\psi$ diminishes.
...
The parameter combinations that closely resemble the links between sampling and decision strategies proposed by Hills and Hertwig [-@hillsInformationSearchDecisions2010]---i.e, high values of $\psi$ and $\theta$ for piecewise sampling and round-wise decisions vs. low values of $\psi$ and $\theta = 1$ for comprehensive sampling and summary decisions)---led to overall numbers of sampled outcomes that come close to the meta-analytic median found by @wulffMetaanalyticReviewTwo2018.
Moreover, while parameter combinations that can be understood as falling in between the originally proposed links---i.e., values in the mid-range for both $\psi$ and $\theta$---also led to overall numbers of sampled outcomes that approximate the meta-analytic median, extreme combinations of both models produce numbers that are considerably lower or higher, and thus less plausible from an empirical stance.
...
For high switching probabilities $\psi$, the low-rank outcome was expected to be sampled less often than would be objectively warranted *within each* comparison round, leading to an inflation of the respective round means of the risky prospect.
Accordingly, the top panels show that for each threshold $\theta$ the rates of false choices of the risky prospect increase with the switching probability $\psi$, thereby causing deviation from sampled mean maximization that take the form of an as-if underweighting of rare outcomes.
...
Here, a similar but more severe pattern can be found, showing that sampling error exercises an influence beyond the effects implied by the interplay of the sampling strategy and the round-wise integration/decision strategy.
...
In this case, then, the means of the risky prospect within each comparison round tend to be more deflated the higher the switching probability $\psi$, which increases the risk of falsely choosing the safe prospect.
Again, the resulting choice patterns resemble an as-if underweighting of rare outcomes.
