...
In the domain of decision making, such inputs provide information about the choice alternatives among which people can choose and may therefore be organized in a set $I$ with each element $i \in I$ denoting a choice alternative. 
That is, each element $i \in I$ is assigned some information from the inputs and this information may be used by the mind to learn about the respective choice alternative.
In turn, decision making can be considered an information processing problem which starts from the set $I$, including all choice alternatives, and stops at a proper subset $I' \subset I$, including only the chosen alternatives.
...
Since there is an uncountable number of decisions people confront in the natural world, behavioral decision research routinely abstracts from decisions between particular choice alternatives, e.g., the choice between job offers, political parties to vote for, travel destinations, investment plans, and whatnot.
Rather, with the choice between at least two *prospects* (see below), it studies a case which is counterfactual in that it omits the many particularities of each decision, but retains in the form of prospects the fundamental *properties* that almost all choice alternatives are assumed to possess [@lopesThoughtsPsychologicalConcept1983].^[See @knightRiskUncertaintyProfit1921, for a discussion of the reasonableness of such abstractions.] 
...
That is, any choice alternative that can be fully described by its outcomes and their probabilities is a prospect. 
For illustration, take the choice alternatives in the following example with monetary outcomes: 

\noindent
**Example 1.** 
*A1 gives \$1 with 100%.*
*A2 gives \$10 with 90% and \$0 with 10%.* 
...
Given a choice between 1, ..., k prospects, the inputs from the environment or memory should therefore provide information about the set
$$
I = \{(\Omega_1, \mathcal{F}_1, P_1), ... ,(\Omega_k, \mathcal{F}_k, P_k)\}
\; .
\tag{3}
$$
...
In such cases, people would make *decisions from description* (DfD), since the information from which the mind learns about the choice alternatives---the probability spaces in (3)---are complete descriptions thereof.
Yet, rather often, people make *decisions from experience* [DFE, @hertwigDecisionsExperienceEffect2004], where the mind can learn about the choice alternatives only from a record of past experiences with them, which necessarily provides only incomplete and probable information.
That is, people may have experienced the possible outcomes of a choice alternative when they themselves or others have made the same decision repeatedly in the past, and the record of experienced outcomes for this particular choice alternative is denoted a *sample*. 
...
Of course, all other things being equal, the accuracy of this inference depends on the size of the sample.
<!--For illustration, take the sampled outcomes in the following example, where alternative A2 from Example 1 was chosen ten times without prior knowledge about the possible outcomes of this alternative and their probabilities:  

\noindent
**Example 2.** *A2 gave: \$0, \$0, \$10, \$0, \$0, \$0, \$0, \$10, \$0, \$0*

\noindent
One may now make an inference from the sample about the possible outcomes of choice alternative A2 and their probabilities. 
However, this conclusion is not certain, but only probable, since the relative frequencies of the results will almost surely change as the sample size increases.--> 

To provide a mathematical definition, this paper treats a sample as a collection of independent and identically distributed (i.i.d.) random variables $X$, each of which is a measurable function defined on the same probability space:^[Note that this paper considers the counterfactual case where samples are random, although it has been emphasized that in the natural world samples are "virtually never random" [@fiedlerBewareSamplesCognitiveecological2000, p. 660]. To account for non-random sampling, e.g., in the form of sampling biases [e.g., @fiedlerBewareSamplesCognitiveecological2000] or dynamic information structures [e.g., @cohenEffectPerceivedPatterns2021; @plonskyRelianceSmallSamples2015], the i.i.d-assumption would need to be dropped, which is out of the scope of this paper.] 
$$
\begin{aligned}
  X : (\Omega, \mathcal{F}) &\to (\Omega', \mathcal{F}') \\
  \omega &\mapsto \omega  \quad \text{for all} \ \omega \in \Omega
\end{aligned}
\tag{4}
$$
[see @kolmogorovFoundationsTheoryProbability1950, p. 21; see also @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, Chapter 1.3, for a more accessible and comprehensive treatment]. 
Accordingly, the inputs to DfE 
$$
S = \{X_{1i}, ... , X_{ki}\}
\; ,
\tag{5}
...
The choices people actually make between prospects---be that on the basis of ---are often described in terms of deviations from expected value (EV) maximization, where the EV is given by 
$$
EV = \sum_i^n \omega_i \times P(X = \omega_i)
\; .
\tag{6}
$$ 
That is, following the principle of EV maximization, one would first calculate the prospects' EV according to (6) and thereafter chose the prospect with the largest EV.
To *describe* how peoples' choices deviate from EV maximization [see @erevAnomaliesForecastsDescriptive2017, for a recent replication of classical demonstrations], the models of cumulative prospect theory [CPT, @tverskyAdvancesProspectTheory1992; see @stottCumulativeProspectTheory2006, for an overview of models] fit choice data with a combination of two parameterized functions that may replace the objective outcomes and probabilities in (3) by nonlinear transformations thereof.^[Note that CPT or other originally deterministic decision models may be amended by a stochastic choice function [see, e.g., @rieskampProbabilisticNaturePreferential2008].] 
...
\tag{5}
$$ 
with subjective values $v(\omega)$ derived from a *value function* that may transform the objective outcomes, and decision weights $\pi$ derived from the objective probabilities' cumulative distribution function that may be transformed by a *probability weighting function* (see below). 
Now, since the probability weighting function may redistribute the entire probability mass of $\sum_i^n p(\omega_i) = 1$ across outcomes, the decision weights 
$$
\pi =
  \begin{cases}
     w(P(X \geq \omega_i)) - w(P(X > \omega_i)) \quad \text{for} \ 1, \\
     w(P(X \leq \omega_i)) - w(P(X < \omega_i)) \quad \text{else}, 
  \end{cases}
\tag{5}
$$
may 
people's deviations from EV maximization is often described in terms of the probability weighting pattern. 

Now, in rather general terms, if people were following the principle of EV maximization, the estimates for the parameters of both functions (see below) would be such that the resulting subjective values and decision weights are equal to the objective outcomes and probabilities, i.e., $v(\omega) = \omega$ and $\pi = p(\omega)$. 
However, for deviations from EV maximization, at least one of the equations should not hold.
...
\tag{4}
$$ 
where $a \in [0,1]$ and $\lambda > 1$ [but see @stottCumulativeProspectTheory2006, for alternative functional forms], transforms the objective outcomes $\omega$ into subjective values $v(\omega)$.
The *probability weighting function*, for which several functional forms have been proposed [e.g., @goldsteinExpressionTheoryPreference1987, see also Equation 5]  
$$
w
\tag{5}
$$ 
transforms the cumulative distribution function of the objective probabilities to derive the subjective decision weights.
...
```{r}
wf <- tibble(gamma = seq(.1, 1.9, .05)) %>%
  expand_grid(prob = seq(0, 1, .01)) %>%
  mutate(w = (prob^gamma)/((prob^gamma)+(1-prob)^gamma))
