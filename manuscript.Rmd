...
Given that the properties of choice alternatives serve as *selection criteria* and the mind is indeed sensitive to variations in their values, the more accurately these properties are described, the more systematic the choices should become.
Importantly, this principle is not restricted to any one specific property, but rather does the choice pattern reflect the bias that the designation of a property as a selection criterion imposes on the assessment of choice alternatives.

As the discussion proceeds and for the sake of example, I argue that a choice pattern robustly emerging in so-called *decisions from experience* (DfE), i.e., the underweighting of small probability events, may be due to the constraints set by a certain selection criterion. 
Considering in some detail *sampling strategies* as input representations used to describe the property of interest, I illustrate that if the sample size increases and thereby uncertainty is reduced, property induction should become more accurate and underweighting more pronounced.
Interestingly, this prediction diverges from the common theme that an increase in the sample size will reduce the underweighting. 
The sampling and decision process are mathematically formalized and implemented into a computational model to simulate choice data [cf. @guestHowComputationalModeling2021; @haslbeckModelingPsychopathologyData2021].
Simulation data is analyzed using descriptive statistics and cumulative prospect theory (CPT) as a commonly used data model [@tverskyAdvancesProspectTheory1992; @goldsteinExpressionTheoryPreference1987]. 
