...
Therefore, decision making can be understood as an information-processing problem whose solution requires a selection among choice alternatives.
...
empirical choice patterns in DfD deviate from the principle of expected value (EV) maximization (see below), as if people give more weight to small-probability outcomes than would be warranted by their objective probabilities;
in turn, in DfE, ceteris paribus, people choose as if the same small-probability outcomes are given less weight than would be objectively warranted [e.g., @barronSmallFeedbackbasedDecisions2003; @hertwigDecisionsExperienceEffect2004; @weberPredictingRiskSensitivity2004; @erevAnomaliesForecastsDescriptive2017; see @wulffMetaanalyticReviewTwo2018, for a comprehensive meta-analytic review].
...
Similar to other behavioral phenomena that can be explained by assuming that the mind is sensitive to the composition of the *samples*---i.e., sets of sampled outcomes---on which it operates [see @fiedlerBewareSamplesCognitiveecological2000], the as-if underweighting of rare outcomes in DfE can be explained by assuming that the mind is sensitive to the information provided by the small samples that people tend to rely on [see @foxDecisionsExperienceSampling2006; @hertwigDecisionsExperienceEffect2004; @hertwigDecisionsExperienceWhy2010; @rakowBiasedSamplesNot2008; @wulffMetaanalyticReviewTwo2018].
...
In such a case or in the less strict case of large samples, then, the binomial distribution of each outcome can be approximated by a symmetric normal distribution.^[<!--Open footnote-->The binomial distribution of an outcome is the probability distribution of the number of times an outcome occurs in a sample. According to the *de Moivre-Laplace theorem* [see, e.g., @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015], the binomial distribution can be approximated by a normal distribution given a large number of sampled outcomes. Respectively, for large samples, the relative relative frequencies with which an outcome occurs in the sample is predicted to correspond to the objective probability of the outcome most of the time and is not predicted to be lower more often than higher and vice versa.<!--Close footnote-->]
...
As a consequence, were the mind to use the sampled relative frequencies of outcomes to infer their latent objective probabilities---without correcting for the sampling error---and to follow the principle of EV maximization, the reliance on small samples could explain the as-if underweighting of rare outcomes.

Yet, studies show that the as-if underweighting of rare outcomes may appear, even if the overall number of sampled outcomes is large and the relative frequencies across all sampled outcomes correspond closely to the latent objective probabilities [@hauDescriptionexperienceGapRisky2008; @hauDecisionsExperienceStatistical2010; @ungemachAreProbabilitiesOverweighted2009; see also @wulffMetaanalyticReviewTwo2018].
Respectively, in the current paper it is assumed that there may be a discrepancy between the overall number of sampled outcomes and the size of the samples that the mind relies on in the evaluation of prospects.
However, in contrast to other approaches, it is not assumed that some of the sampled outcomes are discounted [see, e.g., @hertwigDecisionsExperienceEffect2004, who assume recency effects] or ignored [see, e.g., @cohenEffectPerceivedPatterns2021; @plonskyRelianceSmallSamples2015, who assume that only outcomes that followed some sequential pattern are considered].
Rather, assuming that any number of outcomes sampled from a prospect may either be integrated into the same sample or into multiple smaller samples, this paper presents a computational model that shows that---given a reliance on multiple small samples---an as-if underweighting of rare outcomes can occur, even if a large number of sampled outcomes is fully accounted for.
...
In turn, for a round-wise strategy, it is assumed that over multiple rounds one outcome from each of the two prospects is sampled and that the prospect is chosen for which in most of the rounds the higher outcome was sampled. 
...
Using the proposed model, this paper shows in a computational analysis that an as-if underweighting of rare outcomes can occur even if a large number of sampled outcomes is fully accounted for.
Specifically, considering choices between a two-outcome prospect and a safe prospect, the model is used to simulate the accumulation process for different combinations of switching probabilities and thresholds.
Considering choice rates and parameter estimates of cumulative prospect theory [CPT, @tverskyAdvancesProspectTheory1992; @wakkerAxiomatizationCumulativeProspect1993], the analysis shows that the choices converge to the solutions of EV maximization for low switching probabilities, but take the form of an as-if underweighting of rare outcomes for high switching probabilities.
Importantly, the analysis also shows that these differences between switching probabilities remain stable with increasing thresholds.
...
Before presenting the model and the computational analysis, the following section provides a formal definition of prospects and reviews how CPT can be used to capture deviations from EV maximization.
...
In addition, previous research has shown that differences as to how information about the properties of prospects is processed can translate to characteristic shapes of CPT's value and weighting function [see @pachurHowTwainCan2017; @zilkerNonlinearProbabilityWeighting2021; @stewartDecisionSampling2006].
...
The thresholds therefore serve to implement a speed-accuracy trade-off of the form that whatever systematic differences in the latent properties of prospects may be represented in a mean comparison, should be more accurately assessed as the thresholds increase.
Hence, the as-if underweighting of rare outcomes that is predicted for high switching probabilities is predicted to remain and become more robust the higher the thresholds are.
Respectively, the model should substantiate the claim made at the outset of the paper,
namely, that the reliance on multiple small samples---in the form of pairs of short stochastic processes---can cause an as-if underweighting of rare outcomes while fully accounting for a large number of sampled outcomes.

#### Cumulative Prospect Theory

Since CPT is a statistical or data model designed to capture deviations from the principle of EV maximization, the predicted effects of the model parameters $\psi$ and $\theta$ on the final choices may also translate to the estimates of the parameters in CPT' value and weighting function.
Hence, this section elaborates how the differences in the predictions made for the model parameters may be described in CPT' parameters.
Therefore, the value function of Tversky and Kahneman [-@tverskyAdvancesProspectTheory1992, see Equation (8)] and the weighting function of Goldstein and Einhorn [-@goldsteinExpressionTheoryPreference1987, see Equation (10)] are used.

Now, in the previous two sections, it was predicted that switching probabilities change the length of the stochastic processes underlying each mean comparison and that these changes cause changes as to which prospect is chosen. 
Specifically, for low switching probabilities it was predicted that the stochastic processes are rather long and cause a mean comparison to be indicative for differences in the expected value, thereby resembling the summary strategy.
In turn, for high switching probabilities it was predicted that the stochastic processes are rather short and cause a mean comparison to be indicative for which prospect produces the higher outcome most of the time, thereby resembling the round-wise strategy.

Because for low switching probabilities the prospect should be chosen that possesses the larger expected value, we predict the parameters of the value and weighting function to be in line with a linear weighting, therefore to be close to 1.
Now, because for high switching probabilities the prospect is chosen that produces the greater outcome most of the time, it is predicted that small-probability outcomes are underweighted and and high-probability outcomes are overweighted.
Respectively, it is assumed that $\gamma$ increases with the switching probability.
More specifically, because each comparison is weighted equally the prospect should win, that produces the higher outcome with a probability greater than chance. 
Accordingly, $\delta$ should be such that cumulative probabilities smaller (greater) than $.5$ are underweighted (overweighted).  
