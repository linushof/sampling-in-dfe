...
Often, however, these inputs do not readily describe the *properties* that the operating system uses to control the output. 
...
Since how the input is represented determines the information that is made explicit and the costs at which certain operations can be carried out on that information [@marrVisionComputationalInvestigation1982], the feasibility and ease of these inferences depends greatly on the choice of *representation*.
...
Hence, to explain the operations that turn an input into an output, one needs to understand the properties that control the output, the representation that is used by the cognitive system to describe these properties, and the costs of making the descriptions in that representation, including their susceptibility to distortions from noise or uncertainty. 
