...
The model predicts a decrease in the overall number of sampled outcomes with increases in the switching probability, whereas the overall number of sampled outcomes is predicted to increase with thresholds at relatively constant rate.
...
Because choices become more systematic with increasing switching probabilities, randomness is likely to be caused by the temporal advantage that one of the prospect gains by chance.
...
Rather they affect whether approximately the same number of outcomes is sampled from both prospects or whether one of the prospects obtains, by chance, a temporal advantage, rendering decision either systematic or unsystematic, respectively.
...
Relying on the sequential sampling framework, the current paper assumed that this search process takes the form of an evidence accumulation over time, where outcomes are sampled until an evidence threshold is reached.
Two different models were presented, each of which assumed an accumulation process that resembles a distinct decision strategy.
The round-wise integration model assumed that the means across sets of sampled outcomes are repeatedly compared and a choice is made once the number of won comparisons reaches a threshold.
The summary integration model assumed that all outcomes sampled from a prospect are integrated into the same cumulative sum and that a choice is made once the sum reaches a threshold.
...
Specifically, in the round-wise integration model, changes in the sampling strategy alter the degree to which evidence is accumulated for differences in either the expected values or in the probability to produce a higher outcome most of the time.
If the prospect that produces the higher outcome most of the time does not also possess the higher expected value, changes in the sampling strategy come to imply systematically different choice patterns.
The simulations showed that high switching probabilities led the round-wise integration model to produce systematic deviations from EV maximization that take the form of an as-if underweighting of rare outcomes, even if sampling error is controlled for. 
In contrast, in the summary integration model, changes in the sampling strategy do not cause  alter which latent property evidence is accumulated for, but rather 

Finally, it was shown how the effects of the potential interplay between sampling and integration/decision strategies on the final choice can translate to characteristic signatures in cumulative prospect theory's value and weighting function. 
...
## Why Particular Sampling and Decision Strategies Could Be Linked?
