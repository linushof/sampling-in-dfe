...
fr_rates %>%
  ...
  geom_line(data = fr_rates_m, aes(group = type), color = "gray") + 
  geom_point(data = fr_rates_m, aes(shape = type), color = "gray", size = 2) +
...
The predictions of the choice patterns and CPT parameters alone do not render the model theoretically or empirically meaningful. 
It is right that cognitive models are underdetermined by "locally obtained" data and that they should not aim to merely explain effects---e.g., the as-if underweighting of rare outcomes---but fundamental capacities such as learning, reasoning, and decision making [@vanrooijTheoryTestHow2021].
In explaining these capacities, cognitive models must be constrained to the extent that neither the computational resources of the human mind nor other relevant resources---e.g., time---are limitless [see, e.g., @jareckiFrameworkBuildingCognitive2020; @liederResourcerationalAnalysisUnderstanding2020; @vanrooijTheoryTestHow2021].
...
*ecological rationality* holds that the mind uses simple (decision) heuristics that exploit the information structures of the environment in which it operates [see, e.g., @payneAdaptiveStrategySelection1988; see also @simonRationalChoiceStructure1956];
sequential sampling models assume that the mind trades of processing speed and accuracy while accumulating evidence for or against choice alternatives [see, e.g.,  @bogaczPhysicsOptimalDecision2006; @ratcliffComparisonSequentialSampling2004];
and *resource rationality* assumes that the mind employs mechanisms that make optimal use of its actual resources in obtaining a specific goal [see @liederResourcerationalAnalysisUnderstanding2020].

Admittingly, although the current paper ought to sketch how the mind might exercise the capacity of making decisions from experience, it dealt with these constraints and frameworks rather implicitly and loosely.
The goal of this discussion therefore is to make some of the relations to the existing frameworks for the study of decision making more explicit. 
Most importantly, this paper showed how the assumption that the sampling process is not random  
