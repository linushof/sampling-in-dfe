...
Moreover, because the mean across the entire sampling and accumulation process is expected to converge to the EV the more outcomes are sampled overall, the choice patterns with and without sampling error accounted for should resemble each other more closely, the higher the thresholds get.

To summarize the choice predictions and simplify: 
If the prospect that is more likely to return a higher outcome most of the time does not also possess the higher EV or mean across all outcomes sampled over the course of a sampling and accumulation process, high values of the switching probability $\psi$ are predicted to cause an as-if underweighting of rare outcomes pattern.
...
<!--Close footnote-->] 
This could introduce interactions between the switching probability $\psi$ and the threshold $\theta$ that are not implied by a model that assumes comparisons between means.
...
Accordingly, one might expect that the implied choices converge to the solutions of EV maximization, the larger the thresholds get.
However, the switching probabilities $\psi$ should introduce temporal imbalances between prospects regarding the number of sampled outcomes over which the sums are computed (and compared, assuming relative thresholds, respectively).
...
Specifically, the temporal imbalances---i.e., the temporal advantage for the prospect which is first sampled from---grows as the switching probabilities decrease.
...
This random choice behavior should diminish with increases in the switching probability and choices should converge to the solutions of EV maximization the higher the thresholds get.
...
This pattern should be alleviated, if sampling error is taken into account.
...
Before elaborating how the expectations outlined above may translate into estimates of the CPT parameters, the following section reviews how CPT in general and its weighting function in particular capture deviations from EV maximization.
...
Figure\ \@ref(fig:sample-sizes) displays the number of outcomes sampled over the course of a sampling and accumulation process for each combination of the switching probability $\psi$ and the threshold $\theta$.
The model predicts a decrease in the overall number of sampled outcomes with increases in the switching probability.
As can be seen from the changes in the slopes of the gray lines of the smoothed functions, the magnitude of this decrease diminishes as the relative increase in the switching probability diminishes.
The overall number of sampled outcomes increases with thresholds at a relatively constant rate.
...
The parameter combinations that closely resemble the links between sampling and decision strategies originally proposed by Hills and Hertwig [-@hillsInformationSearchDecisions2010]---i.e, high switching probabilities and thresholds for piecewise sampling and round-wise decisions vs. low switching probabilities and a single mean comparison ($\theta = 1$) for comprehensive sampling and summary decisions---lead to overall numbers of sampled outcomes that come close to the meta-analytic median.
Moreover, while parameter combinations that can be understood as falling in between the originally proposed links---i.e., mid-range values for both switching probability and threshold---also lead to overall numbers of sampled outcomes that approximate the meta-analytic median, extreme combinations (e.g., low switching probability and high threshold) produce numbers that are considerably lower or higher, and thus less plausible from an empirical stance.
...
\raggedright \textit{Note.} Columns represent different thresholds. x-axes display the switching probability. Each point represents a single sampling and accumulation process (choice trial) for the respective combination of the threshold and switching probability, and is the overall number of outcomes sampled over the course of this process. The gray lines are estimates of \textit{ggplot2}'s \texttt{stat\_smooth()} function. All gray-dashed horizontal lines represent the meta-analytic median, med = 14, for choices between a safe and a risky prospect, found by \protect\hyperlink{ref-wulffMetaanalyticReviewTwo2018}{Wulff et al., 2018}. Note that for better resolution the scales on the y-axis differ for different levels of \(\theta\).
\end{figure}

#### Final Choice

For each combination of the switching probability and the threshold, Figure\ \@ref(fig:choice-rates-roundwise) displays the rates of choices that did not maximize the sampled mean (i.e., the mean across all outcomes sampled over the course of a sampling and accumulation process, hereafter: false response rates).
...
In turn, rates of false safe choices remain generally low.
...
Importantly, note from the reduced false response rates in the bottom panel that the inflation and deflation of the means for high switching probabilities must be significantly attenuated when a prospect possesses no small-probability outcome.
...
For $\theta > 1$, the curvature parameter $\gamma$ takes values $\geq 1$ which increase with the switching probability, resulting in a increasingly pronounced S-shaped weighting function.
...
As expected, the elevation parameter $\delta$ takes no values $< 1$, however, the estimates increase slightly with the switching probability, causing the overweighting of the high-rank outcome of the risky prospect to extend across the mid-point of the probability scale.
...
To account for the implied choice behavior, CPT might have to overweight high-rank outcomes with a probability far larger than .5 less severely than high-rank outcomes with a probability only little larger than .5. 
Such a pattern is implied for $\delta > 1$.
Aside this open issue, the estimates for the $\gamma$ and $\delta$ parameter show that the choices implied by the potential interplay of different sampling strategies and the round-wise integration/decision strategy can lead to distinct signatures in CPT's weighting function (see Figure\ \@ref(fig:function-forms), for the resulting graphs of the weighting function).
...
The choice behavior exercised by the summary integration model is less systematic than that of the round-wise integration model, especially for low switching probabilities.
...
Because the choices become more systematic with increasing switching probabilities, randomness is likely to be caused by the temporal advantage that one of the prospect gains by chance.
...
As indicated by the gray smoothed functions, the number of sampled outcomes is generally small and reaches the meta-analytic median found by @wulffMetaanalyticReviewTwo2018 only for higher thresholds.
The switching probability is predicted to exercise no or (at most) a negligible effect on the number of sampled outcomes.

#### Final Choice

For each combination of switching probabilities and the thresholds, Figure\ \@ref(fig:choice-rates-summary) displays the rates of choices that did not maximize the sampled mean.
As for the round-wise integration model, rates are separated for presence and rank of small-probability outcomes.
Consider first the bottom panel which displays the choice rates for all choice problems that do not include an outcome with a probability $< .2$.
Here, the false response rates decrease with increases in the switching probability, going from random choice behavior at rates around .5 for low switching probabilities to a systematic maximization of the sampled mean for high switching probabilities.
Because sampling error and a systematic underrepresentation of small-probability outcomes is accounted for, a similar pattern would be expected for the choice problems that include outcomes with a probability $<.2$.
In principle, this is the case, however, choice behavior appears to be more random over a wider range of switching probabilities.
...
First, note that the estimates for the choice sensitivity parameter $rho$ tend to be low for small switching probabilities (see Fig.\ \@ref(fig:sensitivity-parameters)), implying that for these cases the choices are random and thus, more or less insensitive to the evaluations of the core CPT model. 
Interpretation of CPT parameter estimates for small switching probabilities should therefore be made with great caution (if at all).
