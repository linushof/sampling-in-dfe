...
Yet, in *decisions from experience* [DFE, @hertwigDecisionsExperienceEffect2004], the same properties are latent and therefore not known with certainty, but they must be inferred from the relative frequencies with which the outcomes occurred in the past, e.g., when the prospect was chosen in similar past decisions.
...
In other words, in DFD, people choose as if small-probability outcomes are more probable than they objectively are, and in DFE, people choose as if the same small-probability outcomes are less probable.
...
Yet, the description-experience gap might not be solely caused by the reliance on a small number of sampled outcomes and the associated differences in the information underlying the decision-making processes in DFD and DFE [see also @hauDecisionsExperienceStatistical2010; @hauDescriptionexperienceGapRisky2008; @ungemachAreProbabilitiesOverweighted2009; @wulffMetaanalyticReviewTwo2018, who show that the as-if underweighting of rare outcomes pattern may appear, even if the overall number of sampled outcomes is large and the relative frequencies closely resemble the latent objective probabilities].
Rather, the specifics of the decision-making processes themselves may contribute to the gap.
To this end, the current paper models how the mind might exercise the capacity of making decisions from experience in the sampling paradigm and examines the choice patterns that result from the modeled decision-making processes.
More specifically, controlling for sampling error---i.e., considering the sampled relative frequencies rather than the latent objective probabilities---, this paper investigates in a computational analysis whether and how the interplay of previously proposed sampling and decision strategies [see @hillsInformationSearchDecisions2010, see also below] shapes DFE, and comes to imply choice patterns and functional forms in cumulative prospect theory [CPT, @tverskyAdvancesProspectTheory1992] that are consistent with an as-if underweighting of rare outcomes pattern.^[<!--Open footnote-->
Note that CPT's value function is also considered. However, the focus of this paper is on the weighting function, as it is commonly used to describe an as-if overweighting and underweighting of rare outcomes, as well as the description-experience gap [see, e.g., @kellenHowVariantAre2016; @regenwetterConstructbehaviorGapBehavioral2017; but see @hertwigConstructbehaviorGapDescription2018].
<!--Close footnote-->]
...
To approach the above questions, this paper presents a model that integrates two proposed sampling and decision strategies [see @hillsInformationSearchDecisions2010] into the computational framework of sequential sampling models of decision-making [see, e.g., @markantModelingChoiceSearch2015].
