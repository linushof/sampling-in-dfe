...
Given that each choice alternative is an element $i$ of the set $I$, the basic structure of decision making may be modeled by the inclusion map
$$
\begin{aligned}
  \iota : I' &\to I \\
  i &\mapsto i \quad \text{for all} \ i \in I' 
\end{aligned}
\tag{1}
$$
[@jostMathematicalConcepts2015, pp. 14-15], indicating that decision making is an information processing problem which starts from an input set $I$, including all choice alternatives, and stops at a proper subset $I' \subset I$, including only the chosen alternatives.
...
However, information about the choice alternatives may come in many forms.
Rarely in their daily life, people make *decisions from description*, since in such a case all information about the choice alternatives is available through symbolic descriptions of their *properties*. 
Yet, rather often, people make *decisions from experience* [@hertwigDecisionsExperienceEffect2004], since 
...
\tag{2}
$$ 
$\mathcal{F}$ is the set of all possible subsets of $\Omega$, i.e., the power set $\mathcal{P}(\Omega)$.^[Note that this paper considers only the case of finite sample spaces. The exact axiomatic definition of $\mathcal{F}$, which can be found in Kolmogorov [-@kolmogorovFoundationsTheoryProbability1950, pp. 2-3 and 14-15], also covers infinite sample spaces and is that of a $\sigma$-algebra. For the finite case, however, $\mathcal{P}(\Omega)$ can be considered as such [@georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, Chapter 1.1].] 
...
\tag{3}
$$
which assigns each subset $A \in \mathcal{F}$, also referred to as *events*, a probability $0 < p(A) \leq 1$ with $P(\Omega) = 1$.^[The probabilities $p(\omega)$ are determined by the probability mass function $p : \Omega \mapsto [0,1]$, which is used to construct the probability measure $P(A) = \sum_{\omega \in A} p(\omega)$ for all $A \in \mathcal{F}$ [@georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, Chapter 1.2].]

The choices people actually make between such prospects are often described in terms of deviations from expected value (EV) maximization, where the EV is given by 
$$
EV = \sum_i^n \omega_i \times P(X = \omega_i)
\; ,
\tag{4}
$$ 
with $X$ being a random variable defined on the respective probability space.^[$X$ is measurable function $X: \Omega \to \Omega'$ [see @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, Chapter 1.3] with $\omega \mapsto \omega$ for all $\omega \in \Omega$.]
That is, following the principle of EV maximization, one would first calculate the prospects' EV according to (3) and thereafter chose the prospect with the largest EV.
