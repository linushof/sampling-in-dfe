...
Yet, in *decisions from experience* [DfE, @hertwigDecisionsExperienceEffect2004], the properties of the choice alternatives are latent---i.e., not described---and therefore not known with certainty, but they must be inferred from the consequences (hereafter: outcomes) that occurred when the alternatives were chosen in similar past decisions.
...
In the section that follows, sampling strategies are treated.
...
Following the @kolmogorovFoundationsTheoryProbability1950 axioms, $\Omega$ is the sample space containing a finite set of elements, one of which results from choosing the respective prospect:
$$
...
That is, the set consisting of a single outcome $x$ is a subset $\{x\} \in \mathcal{F'}$ and we therefore obtain $p_X(x) = P(X(\omega) = x)$.
...
In such cases, people would make DfD, since the information from which the mind learns about the prospects are complete descriptions thereof.
Yet, rather often, people make DfE, where the mind can learn about the latent prospects only from a record of past experiences with them.
...
Now, more formally, let the sample of a prospect be a set of realizations of the respective random variable according to its distribution $P_X$.
That is, for the choice between $1, ..., k$ prospects, the inputs to DfE are generated by a set $S$ of stochastic processes
$$
S = \{X_{1t}, ... , X_{kt}\}
\tag{8}
$$
with $t \in \{1, ..., N\}$ denoting a sequence of independent and identically distributed (i.i.d.) random variables and $N$ denoting the length of the respective sequence [but see, e.g., @plonskyRelianceSmallSamples2015, where the i.i.d.-assumption is dropped].
So, given a stochastic process $s \in S$, the obtained realizations---i.e., the experienced outcomes---may be sequentially integrated into a sample of size $N$ or into multiple smaller samples.
Then, the samples so obtained can serve probabilistic inferences about the properties of the latent prospects in (7), the sets of outcome-probability pairs respectively.

Of course, the laws of large numbers [LLN, see, e.g., @kolmogorovFoundationsTheoryProbability1950] apply to DfE and it follows that the accuracy of inferences from samples to latent prospects depend, ceteris paribus, on the size of the samples.
In fact, one explanation for the as-if underweighting of rare outcomes in DfE and the description-experience gap outlined in the introduction is that people rely on too small samples, which, due to the skewness of the binomial distribution, cause the relative frequencies with which rare outcomes occur in the samples to be smaller than their objective probabilities [@hertwigDecisionsExperienceEffect2004; @rakowBiasedSamplesNot2008; see also @hertwigDecisionsExperienceWhy2010].
The invoked argument is then that what may appear as if people were underweighting rare outcomes relative to what would be warranted on the basis of their objective probabilities---following the principle of mathematical expectation---may in fact be consistent with an as-if overweighting relative to what would be warranted on the basis of the experienced relative frequencies [@foxDecisionsExperienceSampling2006].
...
ggplot(wf, aes(p, wp, group = gamma)) +
  ...
  geom_abline(intercept = 0, slope = 1, color = "gray", linetype = "dashed", size = 1) # identity line
```
...
# determine normative choice objective and experienced
fr_rates_p <- choices_p %>%
  mutate(norm = case_when(ev_ratio > 1 ~ "A", ev_ratio < 1 ~ "B"), # normative choice objective EV
         ev_ratio_exp = round(a_ev_exp/b_ev_exp, 2),
         norm_exp = case_when(ev_ratio_exp > 1 ~ "A", ev_ratio_exp < 1 ~ "B")) # normative choice experienced EV / mean
...
fr_rates_p_o <- fr_rates_p %>%
  ...
  summarise(n = n()) %>%
  mutate(rate = round(n/sum(n), 2), # false response rates
         type = case_when(norm == "A" & choice == "B" ~ "Safe", norm == "B" & choice == "A" ~ "Risky")) %>%
  ungroup() %>%
...
fr_rates_p <- bind_rows(fr_rates_p_o, fr_rates_p_e) %>% 
  mutate(rare = case_when(rare == "none" ~ "Middle",
...
fr_rates_p %>% 
  ggplot(aes(s, rate)) +
  geom_point(aes(color = norm, shape = type)) +
  facet_grid(rare~a, labeller = labeller(rare = label_value, a = as_labeller(labeller_a, default = label_parsed))) + 
