...
Such inputs may be treated as a set $I$ and its elements $i \in I$, the latter being distinguishable from each other on the basis of the *properties* they possess [@jostMathematicalConcepts2015, p. 1].
Often, however, the properties used by the mind to control the outputs are not readily described, but proper descriptions must be inferred from the available information. 
...
(ref:figure1) A sketch of the structure of information processing. To turn inputs into meaningful outputs, the mind is required to describe the relevant properties before they can be evaluated. Descriptions are inferred from the inputs using a representation.    

```{r fig.cap="(ref:figure1)", fig.align='left'}
knitr::include_graphics("manuscript_files/figure-png/figure1.png")
```

In what follows, the discussion will turn to the domain of decision making and elaborate on the concepts touched upon thus far, certainly only a subset of the factors involved in the decision making process, yet central enough to illustrate one rather important mechanism:
