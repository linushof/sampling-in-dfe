...
Now, more formally, let us treat the choice alternatives as elements $c$ of the set $C$.
...
For example, in *decisions from description* (DfD), inputs take the form of explicit and complete descriptions of all relevant properties of the choice alternatives.
Yet, in *decisions from experience* [DfE, @hertwigDecisionsExperienceEffect2004], inputs take the form of *samples*.
In this latter case, the properties of the choice alternatives are not described and therefore not known with certainty, but they must be inferred from the consequences (hereafter: outcomes) that occurred when the alternatives were chosen in similar past decisions.
...
In one reading of the gap, empirical choice patterns indicate that in DfD people choose *as if* a rare outcome is given more weight in the evaluation of a choice alternative than would be objectively warranted by the principle of mathematical expectation; in turn, in DfE, ceteris paribus, people choose *as if* the same rare outcome is given less weight than would be warranted [e.g., @barronSmallFeedbackbasedDecisions2003; @hertwigDecisionsExperienceEffect2004; @weberPredictingRiskSensitivity2004; @erevAnomaliesForecastsDescriptive2017; see @wulffMetaanalyticReviewTwo2018, for a comprehensive meta-analytic review].
Hereafter, these choice patterns are referred to as the as-if overweighting and underweighting of rare outcomes, respectively.
...
The predictions of the model are summarized and fitted in CPT.
...
Of course, the laws of large numbers [LLN, see, e.g., @kolmogorovFoundationsTheoryProbability1950] apply to DfE and it follows that the accuracy of such inferences from a sample to a prospect depends, ceteris paribus, on the size of the samples.
In fact, one explanation for the as-if underweighting of rare outcomes in DfE and the description-experience gap outlined in the introduction is that people rely on small samples, which, due to the skewness of the binomial distribution, cause the relative frequencies with which rare outcomes occur in a sample to be smaller than their objective probabilities [@hertwigDecisionsExperienceEffect2004; @rakowBiasedSamplesNot2008; see also @hertwigDecisionsExperienceWhy2010].
The invoked argument is then that what may appear as if people were underweighting rare outcomes relative to what would be warranted on the basis of their objective probabilities---following the principle of mathematical expectation---may in fact be consistent with an as-if overweighting relative to what would be warranted on the basis of the experienced probabilities [@foxDecisionsExperienceSampling2006].
Yet, it seems that the small size of samples in DfE provides just one and only a partial explanation for the description-experience gap [@hauDescriptionexperienceGapRisky2008; @hauDecisionsExperienceStatistical2010; @ungemachAreProbabilitiesOverweighted2009; see also @wulffMetaanalyticReviewTwo2018].
More so, despite the LLN, the gap need not narrow at all, even for ever increasing sample sizes that are fully taken into account.
Particularly, this paper discusses and demonstrates in a simulation how different strategies of sequentially generating outcomes and integrating them into samples interact with the LLN to produce distinct choice patterns in DfE---including the as-if underweighting of rare outcomes---that tend to stabilize as the sample size increases.
Before so doing, however, the following section reviews how cumulative prospect theory (CPT) can be used as a data model that captures systematic deviations from the principle of mathematical expectation in the parameter estimates of a *value* and a *weighting function*. 
...
the models of CPT [@tverskyAdvancesProspectTheory1992] and similar rank-dependent models [see @stottCumulativeProspectTheory2006, for an overview of models] fit choice data by maximizing^[<!--Open footnote-->Note that CPT may be amended by a stochastic choice rule to account for the probabilistic nature of choices [@rieskampProbabilisticNaturePreferential2008]. Most choice rules [@stottCumulativeProspectTheory2006, for an overview] are functions of the prospects' values $V$ and make the choice of a prospect more probable the greater its $V$ relative to those of other prospects.<!--Close footnote-->]
the value
$$
...
Now, several consequences for the weighting pattern that can be estimated in CPT follow from (12), which are reviewed one by one next:
...
That is, the shape of the graph of $w$ displays in which interval on the cumulative probability scale $[0,1]$ the images of $w$ take values that are greater or smaller than the respective cumulative probabilities---i.e., the preimages---and how much greater or smaller these image values are.
...
```{r w-shapes, fig.cap="(ref:weighting)"}
...
To begin with, the gray-dashed identity lines in Figure\ \@ref(fig:w-shapes) imply a linear weighting pattern.
...
That is, for $\gamma > 1$ (green graphs), the graphs take a S-shape---running below (above) the identity line for small (large) cumulative probabilities---which is more accentuated for larger deviations from $\gamma = 1$.
...
Specifically, because of the S-shaped curvature, the differences between the images $w(p)$ may be smaller than those of the respective cumulative probabilities in the lower and upper part of the cumulative probability scale, but may be considerably greater in the middle part.
As a consequence, the cumulative decisions weights that are retrieved from the images of $w$ are smaller than the respective objective probabilities for small-probability outcomes of either a low rank or a high rank, but are larger for small-probability outcomes of middle ranks.^[<!--Open footnote-->Note that the distinction between outcomes of low/high rank and outcomes of middle rank is unnecessary for two-outcome prospects [see @tverskyAdvancesProspectTheory1992]. However, for prospects with finite many outcomes, this distinction is rather critical. E.g., the fact that CPT may then weigh outcomes of the same small probability differently, is one reason why the description-experience gap should not be equated with a reversed weighting pattern in CPT [see @hertwigConstructBehaviorGap2018]. I.e., neither could the gap then be interpreted in terms of an as-if underweighting (overweighting) of rare events in DfE (DfD), nor can any of the factors so far considered as a contributor to the gap (e.g., sample size, optional stopping, recency, etc.) plausibly explain why not all small-probability outcomes are affected in the same way.<!--Close footnote-->]
Note that for all $\gamma < 1$ (blue graphs) the graph of $w$ takes an inverse S-shape and the entire weighting pattern is reversed.
Moreover, the shape of the weighting function, and thus the weighting pattern, also depends strongly on $\delta$, which affects the overall elevation of the graph.
...
To summarize the preceding paragraphs, CPT can be used as a data model to describe choices between prospects by reducing systematic deviations from the principle of mathematical expectation to the parameter estimates of its value and weighting function.
Moreover, each estimated weighting function captures a weighting pattern.
Although the possible weighting patterns that can be captured depend on the exact parameterization of the weighting function [e.g., @goldsteinExpressionTheoryPreference1987], the latter's graph can usually take a more or less accentuated (inverse) S-shape.
The parameter estimates and the implied graphical shape of the weighting function then translate to a weighting pattern where small-probability outcomes of low or high rank are underweighted (overweighted) in the overall evaluation of a prospect according to (10).
Notably, the distinction between outcomes of low/high rank and outcomes of middle rank can be omitted for two-outcome prospects.
In such a case, the weighting pattern reduces to either an underweighting or an overweighting of rare outcomes in CPT. 
For the description of choices between at most two-outcome gambles, one may then adopt the as-if weighting terminology from above, by stating that people decide as if they were either underweighting or overweighting rare outcomes. 
Nevertheless, it cannot be concluded from the estimated weighting pattern that the mind does indeed perform any of the computations associated with the weighting of subjective values in CPT; rather, the mind processes the inputs in a way that the outputs, i.e., the actual choice patterns, approximate the solutions from the estimated CPT model [see, e.g., @griffithsRationalUseCognitive2015].
