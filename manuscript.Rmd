...
in turn, in DFE, choice patterns deviate from the solutions of EV maximization, as if the same small-probability outcomes are given less weight than would be objectively warranted [e.g., @barronSmallFeedbackbasedDecisions2003; @hertwigDecisionsExperienceEffect2004; @weberPredictingRiskSensitivity2004; @erevAnomaliesForecastsDescriptive2017; @regenwetterConstructbehaviorGapBehavioral2017; see @wulffMetaanalyticReviewTwo2018, for a comprehensive meta-analytic review].
...
Therefore, first consider the finding that in DFE *sampling strategies* were indicative for the final choice.
...
To summarize the choice predictions for $\psi$ and simplify: 
If the prospect that is more likely to return a higher outcome most of the time does not also possess the higher EV, high values of $\psi$ are predicted to cause an as-if underweighting of rare outcomes.
Respectively, while these predictions emphasize the close link between small samples and the underweighting of rare outcomes, they suggest that the reliance on small samples can serve the accumulation of evidence for differences in a latent property.

More so, for an increasing sequence of independent comparisons between single outcomes sampled from two prospects, the relative winning frequency of a prospect should converge against the objective probability of producing a higher outcome in each comparison.
Accordingly, the as-if underweighting of rare outcomes should become more robust as the thresholds $\theta$ increase.
...
All outcomes are assumed to be positive and the safe outcome is assumed to fall between the the low-rank outcome, $x_{low}$, and the high-rank outcome, $x_{high}$, of the risky prospect to omit dominant prospects.
...
\tag{15}
$$
that the possible weighting patterns amount to an over- and underweighting of the high-rank outcome of the risky prospect.
...
The computational model was written in the *R* [@rcoreteamLanguageEnvironmentStatistical2021] and is embodied in the GitHub repository accompanying this paper.
