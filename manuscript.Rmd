...
Prospect $P^X$ would be chosen, if $D^Y_v < \theta \leq D^X_v$ is satisfied, and vice versa.
...
\tag{4}
$$
where $D^X_t$ is the cumulative sum after $t$ outcomes sampled from prospect $P^X$ and resembles a dynamic decision variable.
Assuming that $D^Y_m$ is the cumulative sum after $m$ sampled outcomes from prospect $P^Y$, prospect $P^X$ would be chosen, if $D^Y_m < \theta \leq D^X_t$ is satisfied, and vice versa.
...
\tag{5}
$$
where $D^X_{t,m}$ is the difference between the cumulative sums after $t$ and $m$ outcomes sampled from prospects $P^X$ and $P^Y$, respectively.
