...
Let thus $C = \mathbb{R}$ be the set of all possible outcomes of the comparison of the means over two unordered sequences $X_i$ and $Y_i$ of given lengths $N_X$ and $N_Y$ and  
...
where $p$ is the probability of $X$ winning a single mean comparison and $n$ is the number of comparisons (see [Proof A1][Proof A1: Distribution of the Decision Variable]).
...
## Proof A1: Distribution of the Decision Variable

To proof is that a sequence $D_i = D_1, ..., D_n$ of decision variables with the mapping

$$\begin{equation}
D(c \in C) \in C' = 
  \begin{cases}
    1 & \text{for} & B_1 = \{c \in C: (\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0)\} \in \mathscr{C}  \\
    0 & \text{else},
  \end{cases}
\end{equation}$$

describes a Bernoulli process following a binomial distribution $\mathcal{B}\left( p \left(\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0\right), n\right)$ for fixed sequence lengths $N_X$ and $N_Y$.
Therefore, $p \left(\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0\right)$ must be identified.
To demonstrate that this is the case, we assume independence of all random variables $X_i$ and $Y_i$: 

$$\begin{equation}
  \begin{split}
    p(D = 1) & = p\left( \; B_1 = \{c \in C: (\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0)\} \in \mathscr{C} \; \right) \\ \\
    
     & = \sum_{c=1}^{|B_1|} \left( \;
      p(\overline{X}_{N_X})  \times p(\overline{Y}_{N_Y}) \;
    \right)_{c \in B_1} \\ \\
    
    & = \sum_{c=1}^{|B_1|} \left( \; 
      p\left( \frac{1}{N_X} \sum\limits_{i = 1}^{N_X} X_i(\omega \in \Omega_X)\right) 
      \times
      p\left( \frac{1}{N_Y} \sum\limits_{i = 1}^{N_Y} Y_i(\omega \in \Omega_Y)\right) \;
    \right)_{c \in B_1}
    \; ,
  \end{split}
\end{equation}$$

where the index $k = 1, ..., N_k$ can be paraphrased as follows: 
For given sequence lengths $N_X$ and $N_Y$, $k$ denotes a pair of the two sequences $X_i$ and $Y_i$ with a unique combination of the number of times any possible realization of $X$ and $Y$ occurs and which satisfies the inequation $\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0$.

As the unordered sequence $k_{\omega_1}, ..., k_{\omega_n}$ follows the multinomial distribution $\mathcal{M}(N_X, p(\omega_1), ..., p(\omega_n))$, its probability is determined by the probability mass function

$$\begin{equation}
  \begin{split}
    P(k_{\omega_1}, ..., k_{\omega_n}) &= {N_X\choose k_{\omega_1}, ..., k_{\omega_n} } \prod_{\omega \in \Omega_X} p(\omega)^{k_{\omega}} \\ \\
    & = 
    \; ,
  \end{split}
\end{equation}$$
