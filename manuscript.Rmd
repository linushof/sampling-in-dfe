...
That is, not only we need to know how much outcomes are sampled, but also in what way they are sampled, and how this interplays with the way the obtained information is processed to evaluate the latent prospects.

Yet, the presented approach invites objections, both theoretical and methodological.
The following gaps and problems in Hills and Hertwig's [-@hillsInformationSearchDecisions2010] work stand out:

1. *Lack of formal modeling/unstated theoretical commitments:* The authors offer a verbal description of their theoretical claim that sampling strategies are systematically linked to decision strategies.
Yet, they do not provide a formal specification or computational implementation of their theory.
Such a lack of formal, mathematical or computational modeling can, among others, hide gaps and inconsistencies in the theory [@guestHowComputationalModeling2021; @vanrooijPsychologicalModelsTheir2022].
For instance, although the proposed links between sampling and decision strategies are used to explain the (non)occurrence of an as-if underweighting of rare outcomes pattern, no formal and complete accounts of the potential choice-generating mechanisms---which must contain the proposed links---are provided.
One key characteristic of the sampling paradigm is that people can freely decide when to stop sampling.
...
Theoretical frameworks that do so, e.g., sequential sampling models [e.g., @markantModelingChoiceSearch2015], may, however, make themselves assumptions about the decision-making process that constrain how such a proposed link between sampling and decision strategies can be   
Failing to explicitly spell out theoretical commitments and/or to formally model the decision-making process  

2. *Vague consideration of hybrid models:* In principal, the sampling and decision strategies, as proposed, are independent of each other and can be freely combined---i.e., each instance of a sampling strategy can be combined with each instance of a decision strategy. 

3. *Use of heuristic analysis methods:* 

4. *Strong analysis constraints:*

<!-- Outline -->
The current paper aims to address these four problems.
The rest of the paper is structured as follows:
In the first section, the current approach is described

This paper presents a couple of models that integrate two proposed sampling and decision strategies [see @hillsInformationSearchDecisions2010] into the computational framework of sequential sampling models of decision-making [see, e.g., @markantModelingChoiceSearch2015].
More specifically, controlling for sampling error---i.e., considering the sampled relative frequencies rather than the latent objective probabilities---, this paper seeks to investigate in a computational analysis whether and how the potential interplay of sampling and decision strategies shapes DFE and comes to imply choice patterns and functional forms of cumulative prospect theory [CPT, @tverskyAdvancesProspectTheory1992] that are consistent with an as-if underweighting of rare outcomes pattern.^[<!--Open footnote-->
