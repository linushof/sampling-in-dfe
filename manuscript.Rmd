...
Now, more formally,^[Paragraphs with this beginning usually formalize the ideas of the preceding paragraphs or elaborate on them by using mathematical expressions and concepts. Although I consider these paragraphs important, the major propositions of this paper should be captured despite skipping them.] let us treat the choice alternatives as elements $c$ of the set $C$.
...
Hereafter, we refer to these patterns as the as-if overweighting and underweighting of rare outcomes, respectively.
...
\tag{6}
$$
[see @kolmogorovFoundationsTheoryProbability1950, p. 21-22].^[See also @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, Chapter 1, for a comprehensive yet accessible introduction to random variables.]
...
Of course, the accuracy of such inferences depends, ceteris paribus, on the size of the samples, since from the laws of large numbers [see, e.g., @kolmogorovFoundationsTheoryProbability1950] it follows that the relative frequency with which an outcome occurs in a sample converges to the outcome's objective probability only as the size of a sample increases.
Yet, if the choices that people actually make between prospects in DfD and in DfE are described, the differences that were already outlined in the introduction are found, and the size of samples in DfE provides just one and only a partial explanation for this so-called *description-experience gap* [@hertwigDescriptionexperienceGapRisky2009; see also @wulffMetaanalyticReviewTwo2018, for a comprehensive meta-analytic review].
This paper will address this issue<!-- review-->.
Before that, however, the following section reviews how choices between prospects can be described using cumulative prospect theory's (CPT) *weighting function*.^[This paper also considers CPT's value function, but emphasis is put on the weighting function.]
...
To *describe* how people's actual choices deviate from EV maximization [see @erevAnomaliesForecastsDescriptive2017, for a recent replication of classical demonstrations], the models of CPT [@tverskyAdvancesProspectTheory1992] and similar rank-dependent models [see @stottCumulativeProspectTheory2006, for an overview of models] fit choice data by maximizing the value
$$
