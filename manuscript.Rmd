...
As a consequence, were the mind to use the sampled relative frequencies of outcomes to infer their latent objective probabilities (without correction of sampling error, respectively) and to follow the principle of mathematical expectation, the reliance on small samples could explain the as-if underweighting of rare outcomes.
...
This approach distinguishes sampling patterns along the frequency of switches between prospects during the sampling phase and assumes that only outcomes that were sampled between each switch are integrated into the same sample.
Then, assuming that the means of consecutive samples from different prospects are compared, the choice is determined by the number of won comparisons.
...
More specifically, while the probability of a rare outcome to be sampled increases with the length of the sampling process, an increasing switching frequency causes the rare outcome not to be considered in an increasing number of comparisons, which, however, are the primitive units of evidence in this model.
As a consequence, the as-if underweighting of rare outcomes is not predicted to fade with an increase in the overall number of sampled outcomes, if the switching frequency is high.
...
considering the sampling paradigm, this paper discusses and demonstrates in a simulation how an integration of both outlined approaches can explain distinct sampling effects on choices in DfE.

The paper therefore proceeds as follows:
In the first section, a formal definition of prospects and DfE is provided.
