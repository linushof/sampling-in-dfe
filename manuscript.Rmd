...
Let us therefore treat the choice alternatives as elements $c$ of the set $C$, and decision making as an information-processing problem starting from $C$ and stopping at a proper subset $C' \subset C$ with the inclusion map
$$
\begin{aligned}
  \iota : C' &\to C \\
...
\tag{1}
$$
[cf. @jostMathematicalConcepts2015, p. 15].
...
The inputs to DfE therefore take the form of sets of sampled outcomes (hereafter: samples).^[<!--Open footnote-->Note that a sample is defined as a set, with each of its elements being a sampled outcome (or: observation). Accordingly, aside empty and infinite sets, a sample can contain one or finite many sampled outcomes.<!--Close footnote-->]
...
In such a case, but also in the less strict case where the samples grow large, then, the binomial distribution associated with each prospect's outcome can be approximated by a symmetric normal distribution [cf. @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015].
...
Different approaches linking the sampling process to the choices in DfE can explain the as-if underweighting of rare outcomes by the reliance on small samples.
...
To couple the sampling process with the decision-making process, sequential sampling models [e.g., @bhatiaSequentialSamplingParadoxes2014; @busemeyerDecisionFieldTheory1993; @markantModelingChoiceSearch2015; @ratcliffComparisonSequentialSampling2004] propose that evidence is accumulated over time.
The core assumption shared by these models is that the sampled outcomes are sequentially integrated into one or more dynamic decision variables (or: preference states), and that a choice is made once a decision variable exceeds a threshold in favor for one of the prospects.
Here, the overall number of sample outcomes decreases with threshold values, which can explain an as-if underweighting of rare outcomes [see @markantModelingChoiceSearch2015].

In another approach, @hillsInformationSearchDecisions2010 propose a dependency between sampling patterns and decision strategies.
This approach distinguishes sampling patterns along the frequency of switches between prospects during the sampling phase.
Depending on the decision strategy, then, all outcomes sampled for a prospect are integrated into either the same sample or integrated into multiple, non-overlapping smaller samples, each of which is used for an independent (mean-based) comparison with a sample of another prospect.
Accordingly, the choice is determined by the number of won comparisons.
Here, the core assumption is that a high switching frequency coincides with a larger number of comparisons based on respectively smaller samples, which can explain an as-if underweighting of rare outcomes despite a large overall number of sampled outcomes.
...
Now, considering the sampling paradigm, this paper discusses and demonstrates in a simulation how an integration of both outlined approaches can explain distinct sampling effects on choices in DfE.
The paper therefore proceeds as follows:
In the first section, a formal definition of DfE is provided.
In the subsequent section, cumulative prospect theory [CPT, @tverskyAdvancesProspectTheory1992; @wakkerAxiomatizationCumulativeProspect1993] is reviewed as a data model that can be used to describe choices between prospects in terms of deviations from the principle of mathematical expectation.
...
More specifically, the model sequentially accumulates evidence obtained from ordinal comparisons between the prospects' samples, with different strategies of sampling and integrating the prospects outcomes (hereafter: *sampling strategies*) altering the size of these samples.
Thereafter, choices between a safe and a two-outcome prospect are simulated from the model.
The simulations demonstrate how different sampling strategies and thresholds interact with the LLN to produce distinct choice patterns in DfE that can be captured in the parameters of CPT.
In this regard, one key prediction of the model is that the overall number of sampled outcomes can increase without the underweighting of rare outcomes fading, given the sampling strategy translates this increase into a greater number of samples over which evidence is accumulated, but not samples of greater size.
<!--The last section supplements the rather technical inquiry into the sampling process and its consequences with a theoretical inroad into the question how the idea of sampling strategies can be integrated into the broader study of the computationally limited mind as an adaptive cognitive system.-->
...
To begin with, let the axiomatic definition of a prospect be that of a discrete random variable's distribution [see @tverskyAdvancesProspectTheory1992].
Let therefore the tuple $(\Omega, \mathcal{F}, P)$ be a probability space. 
Following the @kolmogorovFoundationsTheoryProbability1950 axioms and the notation of @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, $\Omega$ is the sample space containing a finite set of elements, one of which results from choosing the respective prospect:
$$
\omega_i \in \{\omega_1, ..., \omega_n\} = \Omega
\; .
\tag{2}
$$ 
$\mathcal{F}$ is the set of all possible subsets of $\Omega$, i.e., the power set $\mathcal{P}(\Omega)$, where each element $A \in \mathcal{P}(\Omega) = \mathcal{F}$ is a subset of $\Omega$.^[<!--Open footnote-->Note that this paper considers only the case of finite sample spaces. The exact axiomatic definition of $\mathcal{F}$, which can be found in Kolmogorov [-@kolmogorovFoundationsTheoryProbability1950, pp. 2-3 and 14-15], also covers infinite sample spaces and is that of a $\sigma$-algebra. For the finite case, however, $\mathcal{P}(\Omega)$ can be considered as such [@georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, Chapter 1.1].<!--Close footnote-->]
...
According to Kolmogorov [-@kolmogorovFoundationsTheoryProbability1950, p. 21-22],^[<!--Open footnote-->See also @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015, Chapter 1, for a comprehensive yet accessible introduction to random variables.<!--Close footnote-->]
the random variable $X$ is the measure
$$
\begin{aligned}
...
An outcome's probability $p_X(x)$ is then provided by the pushforward probability measure
$$
P_X : \mathcal{F'} \to [0,1]
\tag{5}
$$
...
\tag{6}
$$
That is, the set consisting of a single outcome $x$ is a subset $\{x\} \in \mathcal{F'}$ and we therefore obtain $p_X(x) = P(\{\omega : X(\omega) = x \in \{x\}\})$.
...
\tag{7}
$$
with each element $c \in C$ being a distribution $P_X$ of a random variable defined on its own probability space.
Accordingly, each $c$ can itself be considered a set of outcome-probability pairs $(x_i, p_X(x_i)) \in \{(x_1, p_X(x_1)), ..., (x_n, p_X(x_n))\}$.

However, only occasionally in peoples' daily life, inputs take the form of explicit descriptions of all outcome-probability pairs.
...
Accordingly, considering the sampling paradigm,^[<!--Open footnote-->Note that it has been emphasized that in the natural world samples are "virtually never random" [@fiedlerBewareSamplesCognitiveecological2000, p. 660]. See, e.g., @plonskyRelianceSmallSamples2015, who drop the i.i.d.-assumption for DfE.<!--Close footnote-->] 
let a sample of a prospect be a set of realizations of the respective random variable.
More specifically, for the choice between $1, ..., k$ prospects, the inputs to DfE---i.e., the sampled outcomes---are generated by a set $S$ of prospect-specific discrete-time stochastic processes
$$
S = \{\{X_{1t}\}, ... , \{X_{kt}\}\}
\; ,
\tag{8}
$$
where each $s \in S$ is a sequence of independent and identically distributed (i.i.d.) random variables, each $t \in \{t_1, ..., t_N\}$ corresponds to the position of the respective random variable in such a sequence, and $N$ denotes the length of the respective sequence, which corresponds to the overall number of sampled outcomes for this prospect.
Now, one core assumption of this paper is that, for a stochastic process $s \in S$ of length $N$, the mind sequentially integrates the sampled outcomes into either one enumerated sample growing to the size $N$ or into multiple smaller enumerated samples, each corresponding to a unique sub-sequence of s.
...
# The Description of Choices and Weighting Patterns in CPT
...
For DfE, several approaches have been proposed to link the sampling process to the final choice. 
...
One core assumption of these models applied to choices between prospects [see, e.g., @bhatiaSequentialSamplingParadoxes2014; @busemeyerDecisionFieldTheory1993; @markantModelingChoiceSearch2015] is that the sampled outcomes are used to sequentially accumulate evidence for or against the respective prospect as the sequences described in (8) grow.
More specifically, sequential sampling models assume that the values of the sampled outcomes are sequentially integrated into a dynamic decision variable.
Once the value of the decision variable reaches a threshold, the respective prospect is chosen. 
...
In this paper, these two ideas are integrated, including speed accuracy tradeoffs etc. with the idea of a link between sampling and decision-making. 
Specifically, it is assumed that evidence is accumulated over time with a decision variable approaching a threshold, however, the model allows for the possibility that evidence accumulation works not necessarily across the single sampled outcomes but over samples thereof. 
We model the processs as one where evidence is accumulated over multiple ordinal comparisons of samples---their means---respectively, rather than over the raw sampled outcomes. 
We therefore combine all thing. 
We show that each follows a Bernoulli distribution: 
