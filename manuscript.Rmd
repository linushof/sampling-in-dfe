...
Even more so, it has been repeatedly stated that for quite many decisions the mind faces, it is simply not possible to draw on direct descriptions of the properties, since they are part of the distal environment  [e.g., @fiedlerExplainingSimulatingJudgment1996; @hertwigDescriptionexperienceGapRisky2009; see also @brunswikRepresentativeDesignProbabilistic1955; @brunswikPerceptionRepresentativeDesign1956; @kozyrevaInterpretationUncertaintyEcological2021].
...
In general, in cases like the one just considered, where the properties are not distal and all information about them is available, property inferences take the form of a mere transformation, where the symbols from the input are ordered and/or combined according to some transformation rules [cf. @marrVisionComputationalInvestigation1982, p. 20].

In natural world decisions, however, the properties of choice alternatives are often distal and amenable only through their probabilistic relation to proximal cues or samples.
Such a proximal information basis, obtained by experience or, more technically, by *sampling*, is neither consistent nor complete but requires the mind to draw inductive inferences to arrive at property descriptions which then, by definition, are only probable. 
In turn, the "achievement" [@brunswikOrganismicAchievementEnvironmental1943, p. 255] of the mind, i.e., highly probable descriptions, depends in part ... [but see @fiedlerInformationSamplingAdaptive2006, for a selection of papers which goes beyond the mere discussion of statistically representative samples]. 

To adapt (3) such that it fits to the sampling, as it is done for decisions from experience [@hertwigDecisionsExperienceEffect2004], we define sampling as follows: 
