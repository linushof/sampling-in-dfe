...
Now, more formally,^[Paragraphs with this beginning usually formalize the main ideas of the preceding paragraphs or elaborate on them by using mathematical expressions and concepts. Although important, the gist of this paper should be captured despite skipping these paragraphs.] let us treat the choice alternatives as elements $c$ of the set $C$.
...
This paper uses the choice pattern in DfE as an entry point for an inquiry into the sampling process as a consequential part of the information processing upfront choices in DfE.
More specifically, it will be discussed and demonstrated in a simulation how changes to the strategy of sequentially generating outcomes and integrating them into samples (hereafter: sampling strategies) can produce distinct choice patterns in DfE---among them, the as-if underweighting of rare outcomes [see @hillsInformationSearchDecisions2010, for the original formulation of this idea]. 
...
However, if the choices that people actually make between prospects are described, robust differences can be found between the choices in DfD and DfE, and the size of samples in DfE provides just one and only a partial explanation for this *description-experience gap* [@hertwigDescriptionexperienceGapRisky2009; see also @wulffMetaanalyticReviewTwo2018, for a comprehensive meta-anayltic review]. 
...
# The Description of Choices in CPT and As-If Underweighting in DfE
