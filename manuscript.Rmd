...
The basic structure of the problem may be modeled by the inclusion map $\iota : A' \mapsto A$, indicating that decision making is a *process* that starts from an initial set $A$, including all choice alternatives, and stops at a proper subset $A' \subset A$, only including the chosen alternatives. 
Given that this selection process is systematic---as opposed to random---the chosen alternatives in $A'$ possess the same *property* or combination of several properties which the other alternatives in the complementary subset $A \setminus A'$ do not possess.
...
Thus, a rather important part---although by far not the only one---of the computation that connects the initial set with its subset is the comparison of the properties of the choice alternatives against some selection criteria. 
At the algorithmic level, such comparisons require an explicit description of the properties using a representation system which is useful to the processing system, i.e., a living organism or a computer [@marrRepresentationRecognitionSpatial1978; @marrVisionComputationalInvestigation1982]. 
In quite many cases, however, the selection process is not restricted to one particular representation system but a number of different representations can be used to describe the properties of a given set of choice alternatives. 
E.g., an important difference that has emerged in behavioral decision research is whether the probability of a risky outcome is readily described as a decimal number, ratio, or percentage or whether it must be inferred from the relative frequency of occurrence of the particular outcome in a collection of random variables [@hertwigDescriptionexperienceGapRisky2009]. 
