...
In the domain of decision making, such inputs provide information about the choice alternatives among which people can choose, hereafter treated as elements $c$ of the set $C$.
...
with 
$$
P_X(A' \in \mathcal{F'}) := P(X^{-1} \times A') = P(\{\omega: X(\omega) = x \in A'\})
\tag{6}
$$
...
That is, the set consisting of a single outcome $x$ can be treated as a subset $\{x\} \in \mathcal{F'}$ and one therefore obtains $p_X(x) = P(X(\omega) = x)$.
Note that the probability measure in (5) and (6), respectively, is the discrete random variable's distribution and therefore a prospect.
...
-->
More formally, let the sample of a prospect be a set of realizations of the respective random variable $X$ according to its distribution $P_X$.
Accordingly, for the choice between $1, ..., k$ prospects, the inputs to DfE are generated by a set $S$ of stochastic processes
$$
S = \{X_{1t}, ... , X_{kt}\}
\tag{8}
$$
with $t \in \{1, ..., N\}$ denoting a sequence of independent and identically distributed (i.i.d.) random variables and $N$ denoting the size of the respective sequence.^[Note that this paper considers the counterfactual case where samples are random, although it has been emphasized that in the natural world samples are "virtually never random" [@fiedlerBewareSamplesCognitiveecological2000, p. 660]. To account for non-random sampling, e.g., in the form of sampling biases [e.g., @fiedlerBewareSamplesCognitiveecological2000] or dynamic information structures [e.g., @cohenEffectPerceivedPatterns2021; @plonskyRelianceSmallSamples2015], the i.i.d-assumption would need to be dropped, which is out of the scope of this paper.]
...
\tag{9}
$$ 
That is, following the principle of EV maximization, one would first calculate the prospects' EV according to (9) and thereafter chose the prospect with the largest EV.
To *describe* how peoples' choices deviate from EV maximization [see @erevAnomaliesForecastsDescriptive2017, for a recent replication of classical demonstrations], the models of cumulative prospect theory [CPT, @tverskyAdvancesProspectTheory1992] and similar rank-dependent models [see @stottCumulativeProspectTheory2006, for an overview of models] fit choice data with a combination of two parameterized, monotonic functions that may replace the objective outcomes and probabilities in (9) by nonlinear transformations thereof. 
More specifically, the models of CPT fit choice data by maximizing the value 
$$
V = \sum_i^n v(x_i) \times \pi_i
\; ,
\tag{10}
$$ 
with objective outcomes $x_i$ being transformed by a *value function* $v$, and cumulative decision weights $\pi_i$ being derived from cumulative probabilities of the distribution $P_X$ that may be transformed by a *weighting function* $w$ (see (11) and (12) below).^[Note that in CPT, the value and weighting functions are sign-dependent. Accordingly, two value and weighting functions are needed for prospects with positive and negative outcomes, respectively [see @tverskyAdvancesProspectTheory1992]. Note further that CPT or other originally deterministic decision models may be amended by a stochastic choice function to account for the probabilistic nature of choices [see, e.g., @rieskampProbabilisticNaturePreferential2008].]

## Probability Weighting and The Description-Experience Gap

According to @tverskyAdvancesProspectTheory1992, let the cumulative decision weights be defined by
$$
\pi_i =
  \begin{cases}
     w^+(P(X \geq x_i)) - w^+(P(X > x_i)) \quad \forall x \geq 0, \\
     w^-(P(X \leq x_i)) - w^-(P(X < x_i)) \quad \text{else}, 
  \end{cases}
\tag{11}
$$
with $w^+(0) = w^-(0) = 0$ and $w^+(1) = w^-(1) = 1$.
Now, given that $w(P_X(\Omega')) = w(1) = 1$, it follows from (11) that for any prospect that is strictly positive or strictly negative, the weighting function may redistribute the entire probability mass $P_X(\Omega') = 1$ across outcomes [see @zilkerNonlinearProbabilityWeighting2021]. 
As a consequence, the decision weights may deviate---i.e., they may be smaller or greater---from the respective objective probabilities<!-- and these deviations are usually regressive-->.
Although the possible deviation patterns, which span the entire probability scale, depend on the exact parameterization of the weighting function [e.g., @goldsteinExpressionTheoryPreference1987; @tverskyAdvancesProspectTheory1992; @prelecProbabilityWeightingFunction1998; see also @stottCumulativeProspectTheory2006], they allow the adoption of an as-if weighting terminology for the description of choice behavior, which was originally introduced outside of the CPT models [see @pachurWeightUncertainEvents2019].
...
ggplot(wf) +
  ...
  geom_segment(x = 0, xend = 1, y =  0, yend = 1, linetype = "dashed", size = .3) +
