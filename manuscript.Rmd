...
Importantly, this principle is not restricted to any one specific property, but rather does the choice pattern result from the constraints that the designation of a property as a selection criterion imposes on the assessment of choice alternatives.
...
Considering *sampling strategies* as representations used to describe the properties of interest in DfE, I illustrate how the inferred descriptions of these properties indeed become more accurate with increasing sample sizes. 
Yet, caused by the selection criterion corresponding to the round-wise decision rule, the underweighting pattern is a priori predicted to become more pronounced in such a case. 
This prediction is different from the relatively undisputed explanation that underweighting is caused by the reliance on small samples and the skewness of the binomial distribution, indicating that the effects of sample size on underweighting are conditional on the selection criterion. 
I will elaborate on this matter as I proceed, however, to be rather clear from the outset, what the mind does in neither case is performing an underweighting computation in its literal sense, but it processes the inputs in a way that the outputs approximate the solutions of what can be considered probability weighting on an abstract computational level.

The theoretical argument is substantiated by a simulation study [cf. @guestHowComputationalModeling2021]. 
For this purpose, the sampling and decision processes are expressed mathematically and implemented in a computational model.
...
All simulations and analyses are reproducible---materials and instructions can be found on the accompanying GitHub repository.
