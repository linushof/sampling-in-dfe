...
Yet, there are distinct latent properties and combinations thereof that can be used to assess differences between prospects and for which evidence may be accumulated.
Respectively, this paper presents a computational model in which the latent properties, whose differences among prospects are represented in the decision variable, can change.
To this end, the model implements the summary and the round-wise decision strategy---for which either the prospect is chosen that produced the greater mean across all sampled outcomes or the prospect that won more comparisons between single sampled outcomes, respectively [see @hillsInformationSearchDecisions2010]---into a common evidence accumulation model.
...
The model assumes that evidence is accumulated over a sequence of mean-based comparisons between the samples.
