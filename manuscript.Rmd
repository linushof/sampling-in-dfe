...
In such a case, then, according to the de Moivre-Laplace theorem, the binomial distribution associated with each prospect's outcome can be approximated by a symmetric normal distribution [cf. @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015].
...
One class of process models that link the actual sampling process to the decision-making process and can account for the as-if underweighting of rare outcomes by assuming a reliance on small samples, are sequential sampling models of decision making [e.g., @bhatiaSequentialSamplingParadoxes2014; @busemeyerDecisionFieldTheory1993; @markantModelingChoiceSearch2015; @ratcliffComparisonSequentialSampling2004].
...
To relate the sampling process to the final choice, sequential sampling models assume that the information-processing chain upfront the final choice takes the form of an evidence accumulation process over time, with choices becoming more accurate the more evidence must be accumulated.
Specifically, the models assume that the sampled outcomes are sequentially accumulated into one or more dynamic decision variables and that a choice is made once a decision variable exceeds a threshold in favor for one of the prospects.
Now, since the thresholds determine how much (more) evidence a prospect must accumulate in order to be chosen and since the amount of evidence for either prospect can only increase by sampling, these models predict, ceteris paribus, an as-if underweighting of rare outcomes for low thresholds [see @markantModelingChoiceSearch2015].
In other words, a decrease in the threshold leads to a decrease in the number of sampled outcomes that are required to reach this threshold. 
Accordingly, for low thresholds, small-probability outcomes are predicted to contribute less to the accumulated evidence than warranted by their objective probability. 

Yet, some studies found the as-if underweighting of rare outcomes in DfE to appear, even if the overall number of sampled outcomes is large and the relative frequencies across all sampled outcomes closely resemble the latent objective probabilities [@hauDescriptionexperienceGapRisky2008; @hauDecisionsExperienceStatistical2010; @ungemachAreProbabilitiesOverweighted2009; see also @wulffMetaanalyticReviewTwo2018].
While this finding demonstrates that the as-if underweighting of rare outcomes in DfE cannot be reduced to a small overall number of sampled outcomes, it may still be the case that the respective deviations from the principle of mathematical expectation were caused by the reliance on small samples.
For example, given a large overall number of sampled outcomes, an as-if underweighting of rare outcomes could be explained, if only a small subset of all the sampled outcomes was considered for the assessment of prospects.
<!---->
