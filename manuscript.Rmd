...
In the domain of decision making, such inputs can be treated as a set $I$ of choice alternatives $i \in I$, the latter being distinguishable from each other on the basis of the *properties* they possess [cf. @jostMathematicalConcepts2015].
The basic structure of decision making may then be modeled by the inclusion map $\iota : I' \mapsto I$, where $\iota(i) = i$ for all $i \in I'$, indicating that decision making is a *process* that starts from an input set $I$, including all choice alternatives, and stops at a proper subset $I' \subset I$, including only the chosen alternatives. 
Given that the choice is systematic---as opposed to random---the chosen alternatives in $I'$ should possess the same property or combination of several properties which the other alternatives in the complementary subset $I \setminus I'$ do not possess.
...
(ref:figure1) A computational level sketch of decision making as an information processing problem. To make a selection among choice alternatives on the basis of particular selection criteria, the mind requires explicit descriptions of the relevant properties before they can be evaluated. Descriptions are either readily available or must be inferred from the inputs.

```{r fig.cap="(ref:figure1)", fig.align='left'}
knitr::include_graphics("manuscript_files/figure-png/figure1.png")
```

In what follows, the discussion will elaborate on the concepts touched upon thus far, certainly only a subset of the factors involved in the decision making process, yet central enough to illustrate one rather important principle:
...
As the discussion proceeds and for the sake of example, I argue that a choice pattern robustly emerging in *decisions from experience* (DfE), the apparent underweighting of small probability events [e.g., @barronSmallFeedbackbasedDecisions2003; @hertwigDecisionsExperienceEffect2004; @weberPredictingRiskSensitivity2004; see also @wulffMetaanalyticReviewTwo2018, for a meta-analytic review], may be caused by the assessment constraints associated with a round-wise decision strategy.
This particular claim has already been made by @hillsInformationSearchDecisions2010, is in principle not restricted to DfE, and follows quite directly from probability theory.
However, the current article treats decision strategies explicitly as selection criteria and addresses in some detail a few of the problems associated with DfE, in which descriptions of the properties of choice alternatives must be inferred from sampling data.

For one, there is the well-known statistical *induction problem* that the sample size determines the margin of error within which any true property of a choice alternative can be inferred, given an otherwise perfect inference process. 
Yet, the underweighting pattern that is caused by the adoption of a certain selection criterion is a priori predicted to become more pronounced with increasing sample size. 
This prediction is in line with the principle already stated above, i.e., that choice patterns should become more systematic as property descriptions become more accurate.
Nevertheless, the prediction is different from the explanation that underweighting is caused by the reliance on small samples and the skewness of the binomial distribution [e.g., @foxDecisionsExperienceSampling2006; @hertwigDecisionsExperienceEffect2004; @hauDescriptionexperienceGapRisky2008; @ungemachAreProbabilitiesOverweighted2009; @rakowBiasedSamplesNot2008], highlighting that outside the *statistical* models used to describe the choice data, *causal* decision making models of quite different nature may be at play [cf. @mcelreathStatisticalRethinkingBayesian2020], and that a rather important difference between the latter resides in the selection criteria they include.
I will elaborate on this matter as I proceed, however, to be clear from the outset, what the mind does in neither case is performing an underweighting computation in its literal sense, but it processes the inputs in a way that the outputs approximate the solutions of what can be considered probability weighting on an abstract computational level [cf. @griffithsRationalUseCognitive2015]. 

For another, this article approaches the problem of organizing the sampling data in such a way that it allows chains of inferences connecting the input set $I$ with the output set $I' \subset I$ that are algorithmically short and simple enough to be carried out by minds whose cognitive capacities are bounded [see @simonBehavioralModelRational1955; @simonInvariantsHumanBehavior1990]. 
...
In this article, *sampling strategies* in DfE are considered as representation systems used to organize the samples in a way that subserves the property inferences required by the application of particular selection criteria.
...
By restating this idea and adopting some terms and concepts that have well-defined meaning in math and cognitive science, I hope to integrate it into the rich study of the computationally limited mind as an adaptive cognitive system whose processes interact with the information structures in the environment or memory [e.g., @simonRationalChoiceStructure1956; @hertwigTamingUncertainty2019]. 

The theoretical arguments sketched in this admittingly dense introduction are substantiated by a simulation study. 
For this purpose, the sampling and decision making processes are expressed mathematically and implemented in a computational model to simulate choice data from DfE.
...
# The Decision Making Process and the Properties of Choice Alternatives

We already stated that decision making is an information-processing problem whose solution requires a selection among choice alternatives.  all chosen alternatives are equal to each other but different from the non-chosen alternatives.

Now, consider the power set $\mathcal{P}(I)$, which is the set of all possible subsets of $I$ (see example in Figure 2).
Each of the subsets $I' \in \mathcal{P}(I)$ corresponds to a possible distinction between the choice alternatives in $I$ that can be made after assessing for each alternative whether it fulfills the selection criteria or not [cf. @jostMathematicalConcepts2015, p. 16]. 
The concept of the power set thereby illustrates in a plain way how different selection criteria can alter the consequential choice, provided the properties are not uniformly distributed among the alternatives and the mind is sensitive to whatever relations the alternatives have with each other.
\

(ref:figure2) The power set of the set $I$ including the choice alternatives A, B, and C. Black (white) circles indicate that an alternative does (not) possess a property. Each column displays a possible subset that could result from assessing whether the alternatives posses a relevant property. The properties determine the various relations among the choice alternatives. Note that properties resulting in the empty set $\{\}$ and the subset $\{A,B,C\}$ do not discriminate between choice alternatives.
...
impose a proper organization on them, with the rules for doing so determined by the representation [cf. @marrVisionComputationalInvestigation1982; @marrRepresentationRecognitionSpatial1978], or it must make an inference from incomplete information, or both.
Consider a prospect $P$ with the following outcomes (probabilities):
...
In (1), outcomes and probabilities are properties of $P$ and readily described using the decimal number system as a representation.
Now, if the expected value as another property of $P$ is to be described, the organizational rule would be to first multiply the outcomes by their probability and then sum the resulting products, i.e., $8 \times .9 + 2.5 \times .1 = 7.45$.
For the sake of example, one could think of another prospect 
...
where the probability of the outcome $2.5$ must be inferred from the facts that the prospect has only two outcomes and the other outcome has a probability of $.9$. 
Therefore, in (2), the explicit description of the probability as one property of $P$ requires an inference, whereas the description of the expected value as another property now requires both, an inference and a calculus. 
...
## Subheading: Capturing Differences in Sampling and Decision Strategies  
...
## Subheading: Predicting Choice Behavior 
