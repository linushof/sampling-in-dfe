...
The sequential sampling framework is chosen as a modeling outlet not only because it makes parsimonious assumptions and has assembled a good *"track record"* [cf. @meehlAppraisingAmendingTheories1990] by explaining and predicting actual choice behavior, but primarily because it makes explicit a core part of the computational problem the mind is trying to solve: 
in order to make systematic choices, the mind must assess differences in latent properties.
And in accumulating evidence for and against propsects, what should the mind accumulate evidence for if not such differences?
round-wise and summary decisions are both indicative for distinct properties the expected value and the probability to produce a higher outcome most of the time.
The remainder of the paper discusses and demonstrates in a simulation, how sampling and decision strategies as well as thresholds can act together to alter the degree and the accuracy with which these differences are assessed.
