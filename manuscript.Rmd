...
although $p$ can in principle be determined (see [Appendix][A1. Determining the Probability of Success in a Comparison of Sample Means]), it becomes intractable with increasing elements in $\Omega$ and growing sequence lengths. 
...
## A1. Determining the Probability of Success in a Comparison of Sample Means

To proof is that the sequence $D_i = D_1, ..., D_n$ (see [Definition of D][A Stochastic Sampling Model Capturing Differences in Sampling and Decision Strategies]) describes a Bernoulli process following the binomial distribution $\mathcal{B}\left(p(\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0), n \right)$ for fixed sequence lengths $N_X$ and $N_Y$.
...
We start by setting the probability of success equal to the probability of $B_1 \in \mathscr{C}$ which is the subset of the set of all mean comparisons resulting from the pairs $X_i \cap Y_i$ with a unique combination of the number of times any possible realization of $X$ and $Y$ satisfying the inequation $\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0$. 
...
where $|B_1|$ denotes the cardinality of the set $B_1$. 
The above equation can be paraphrased as follows: 
The probability that an unordered sequence $X_i$ of length $N_X$ has a higher mean than an unordered sequence $Y_i$ of length $N_Y$ is given by the sum of the probabilities of all $|B_1|$ pairs $X_i \cap Y_i$ with a unique combination of the number of times any possible realization of $X$ and $Y$ occurs and which satisfies the inequation $\overline{X}_{N_X} - \overline{Y}_{N_Y} > 0$.

Let $k_{\omega}$ be the number of times an outcome $\omega \in \Omega$ occurs in the sequence $X_i$. As the unordered sequence $k_{\omega_1}, ..., k_{\omega_n}$ follows the multinomial distribution $\mathcal{M}(N_X, p(\omega_1), ..., p(\omega_n))$, its probability is determined by the probability mass function
