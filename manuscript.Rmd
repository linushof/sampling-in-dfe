...
The inputs to DfE therefore take the form of samples.
...
In such a case, but also in the less strict case of large samples, then, the binomial distribution of each prospect's outcome can be approximated by a symmetric normal distribution [see @georgiiStochastikEinfuhrungWahrscheinlichkeitstheorie2015].
However, for small samples, the binomial distribution of small-probability outcomes is positive skewed, therefore causing the relative frequencies with which these outcomes occur in the sample to be smaller rather than larger than their latent objective probabilities.
Thus, small-probability outcomes tend to be underrepresented in small samples and might not occur at all.
As a consequence, were the mind to use the sampled relative frequencies of outcomes to infer their latent objective probabilities---without correction of sampling error---and to follow the principle of mathematical expectation, the reliance on small samples could explain the as-if underweighting of rare outcomes. 
Since the same line of reasoning applies would the mind approximate the principle of mathematical expectation by choosing the prospect that produced the greater sample mean, the following discussion focuses on choices in DfE that are based on comparisons of sampled means.
The relevance of making the inference about latent properties explicit, however, is addressed in the last section of this paper. 

Now, this paper uses the outlined effect of small samples as a starting point for an inquiry into the sampling process upfront choices in DfE.
Specifically, in one paradigm used to study DfE, i.e., the sampling paradigm [see @weberPredictingRiskSensitivity2004], before making a single choice between prospects with latent properties, people can, as much as they want, sequentially sample the prospects' outcomes according to their objective probabilities.
Considering this paradigm, the current paper discusses how different strategies of sampling and integrating the prospects' outcomes (hereafter: *sampling strategies*) can produce distinct choice patterns in DfE by altering the number and size of the samples that are used by the mind to accumulate evidence over time [see @hillsInformationSearchDecisions2010, for the original formulation of this claim].
...
In the section that follows, using an evidence accumulation approach, different sampling strategies are discussed, how they produce distinct choice patterns in DfE, and how these patterns may be captured in the parameters of CPT.
Thereafter, a simulation is presented in which these sampling strategies are implemented in a computational model to simulate choices between a safe and a two-outcome prospect.
By proposing distinct links between the parameter capturing different sampling strategies, the resulting choices, and the estimated CPT parameters, the simulation substantiates the core claim of this paper, namely, that sampling effects on choices in DfE may not be reduced to the overall number of sampled outcomes.
That is, the simulation demonstrates how the overall number of sampled outcomes can increase without the underweighting of rare outcomes fading, assuming that the sampling strategy translates this increase in overall number into a greater number of samples over which evidence is accumulated, but not samples of greater size.
...
However, as such, CPT cannot substitute algorithmic-level theories that make claims about how exactly the mind transforms the inputs to the decision making process into outputs---i.e., into choices---which approximate the solutions of CPT's calculus [see @zilkerMeasuringModelingConstruction2020; see also @griffithsProbabilisticModelsCognition2010].
...
The need to distinguish between the computational and the algorithmic level in the domain of decision making is exemplified by the fact that information about prospects may come in the form of descriptions or samples.
Whereas CPT can be used to model choices in DfD and DfE, the information-processing chains upfront choices in DfE cannot be identical to those in DfD.
That is, inputs in the form of experienced outcomes require different algorithms to be processed than do descriptions of outcomes and their probabilities.
Specifically, the mind must integrate the experienced outcomes into a sample in order to make an inference about the properties of the latent prospects, the outcome-probability pairs respectively. 
