...
Now, more formally, let the sample of a prospect be a set of realizations of the respective random variable according to its distribution $P_X$.^[<!--Open footnote-->Note that this paper considers random samples, although it has been emphasized that in the natural world samples are "virtually never random" [@fiedlerBewareSamplesCognitiveecological2000, p. 660]. To account for non-random sampling, e.g., in the form of sampling biases [see @fiedlerBewareSamplesCognitiveecological2000] or dynamic information structures [see, e.g., @cohenEffectPerceivedPatterns2021; @plonskyRelianceSmallSamples2015], the i.i.d-assumption would need to be dropped.<!--Close footnote-->]
...
The realizations that result from each stochastic process $s \in S$ may then be integrated into a sample of $N$ experienced outcomes or into multiple smaller samples.
The samples may then be used to make an uncertain inference about the latent prospects in (7), the sets of outcome-probability pairs respectively.

Of course, the laws of large numbers [LLN, see, e.g., @kolmogorovFoundationsTheoryProbability1950] apply to DfE and it follows that the accuracy of an inference from a sample about the properties of a latent prospect depends, ceteris paribus, on the size of the sample.
In fact, one explanation for the as-if underweighting of rare outcomes in DfE and the description-experience gap outlined in the introduction is that people rely on too small samples, which, due to the skewness of the binomial distribution, cause the relative frequencies with which rare outcomes occur in a sample to be smaller than their objective probabilities [@hertwigDecisionsExperienceEffect2004; @rakowBiasedSamplesNot2008; see also @hertwigDecisionsExperienceWhy2010].
