...
In fact, irrespective of their exact specification, decision models generally take as inputs information about a set of choice alternatives, each possessing some defining *properties*, and return a subset of chosen alternatives as outputs [@heOntologyDecisionModels2020].
...
Yet, in *decisions from experience* [DfE, @hertwigDecisionsExperienceEffect2004], these properties are latent and therefore not known with certainty, but they must be inferred from the relative frequencies with which the outcomes occurred when the prospect was chosen in past consequential or non-consequential decisions.
...
Similar to other behavioral phenomena that could be explained by assuming that the mind is sensitive to the composition of the *samples*---i.e., sets of sampled outcomes---on which it operates [see @fiedlerBewareSamplesCognitiveecological2000], the as-if underweighting of rare outcomes in DfE could be explained by assuming that the mind is sensitive to the information provided by the small samples that people tend to rely on [see @foxDecisionsExperienceSampling2006; @hertwigDecisionsExperienceEffect2004; @hertwigDecisionsExperienceWhy2010; @rakowBiasedSamplesNot2008; @wulffMetaanalyticReviewTwo2018].
...
For example, some studies suggest that outcomes that were sampled more recently receive more weight in the evaluation of a prospect [see @hertwigDecisionsExperienceEffect2004] or that only those outcomes are considered right before a choice that followed some sequential pattern [see @cohenEffectPerceivedPatterns2021; @plonskyRelianceSmallSamples2015].
In either way, the effective sample that is used to evaluate the prospect is smaller than the number of sampled outcomes and thus more prone to underrepresent a small-probability outcome.

However, in contrast to the approaches outlined so far, this paper presents a process model that demonstrates that people may rely on small samples and choose as if they were underweighting rare outcomes while still sampling and taking into account a large number of outcomes.
Therefore, consider first the finding that *sampling strategies* were indicative for the final choice in DfE.
Specifically, considering the choice between two prospects in the sampling paradigm, @hillsInformationSearchDecisions2010 distinguished sampling strategies along the frequency of switches between the two prospects during the sampling phase,^[<!--Open footnote-->Note that @hillsInformationSearchDecisions2010 proposed two paradigmatic sampling strategies, where prospects are either switched just once (comprehensive sampling) or after each sampled outcome (piecewise sampling).
...
To this end, this paper presents a process model that provides a mechanistic link between the *switching probability*---i.e., the probability of sampling outcomes from different prospects in direct succession during the sampling phase---, the decision strategy, and the respective choice.
...
However, to allow for a gradual transition from the summary to the round-wise strategy for increases in the switching probability, the model also captures hybrids of both decision strategies.
Specifically, the model accumulates the evidence obtained from the comparisons between sample means---rather than from single outcomes, as in the sequential sampling models outlined above---with the size of the samples underlying each mean comparison decreasing with an increasing switching probability, and thresholds determining how much (more) of such mean comparisons a prospect must win in order to be chosen.
Thus, the model presented in this paper is such that the higher (lower) the switching probabilities and the higher (lower) the thresholds, the more the evaluation of prospects or the accumulation process, respectively, resembles a round-wise strategy (summary strategy).

Moreover, this paper demonstrates in a simulation that the proposed model can be used to explain how peoples' evaluations of prospects can rely on small samples and lead to an as-if underweighting of rare outcomes, even if the number of sampled outcomes that is taken into account increases.
Specifically, considering choices between a two-outcome prospect and a safe prospect, the model is used to simulate the evidence accumulation process and the respective choices for different combinations of switching probabilities and threshold values.
Then, in a computational analyses using choice rates and parameter estimates of cumulative prospect theory [CPT, @tverskyAdvancesProspectTheory1992; @wakkerAxiomatizationCumulativeProspect1993], it is shown that an increase in the switching probability causes a stronger as-if underweighting of rare outcomes, whereas for decreasing switching probabilities, choices converge to the solutions of the principle of mathematical expectation.
The analyses moreover show that these differences between switching probabilities in the choice rates and CPT parameter estimates they produce remain stable as the thresholds increase.
Accordingly, the results of the simulations and analyses substantiate the claim that the reliance on small samples but not necessarily the amount of sampled outcomes considered during the decision-making process cause an as-if underweighting of rare outcomes.
That is, whereas the thresholds govern the length of the evidence accumulation process, the proposed model allows this process to unfold over mean comparisons between samples that tend to be small for high switching probabilities and large for low switching probabilities.
Now, because in this model each comparison's contribution to the accumulated evidence is weighted equally in the form of round wins but ignores the magnitude of outcomes once a comparison is carried out, differences in the threshold values cannot predict differences in the amount a small-probability outcome contributes to the accumulated evidence relative to what would have been warranted by the principle of mathematical expectation. 
However, because high switching probabilities reduce the size of the samples on which a mean comparison is based, they cause small-probability outcomes to contribute to the outcome of each mean comparison less than would be objectively warranted. 
To this end, then, this paper argues that sampling strategies or switching probabilities, respectively, may serve to alter which differences in the latent properties of prospects are represented in the outcomes of the mean comparisons.
Before presenting the model and the simulation study, the following section provides a formal definition of prospects and reviews how CPT can be used to describe choices between prospects in DfE in terms of deviations from the principle of mathematical expectation. 
...
Roughly speaking, then, all choice alternatives that can be fully described by their outcome-probability pairs $(x_i, p_X(x_i))$ are prospects (or: gambles).
