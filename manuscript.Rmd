...
cols_cpt <- list(.default = col_double(),
                               ...
                               a = col_factor(),
                               parameter = col_factor())
cpt_c <- read_csv("supplements/estimates_cpt_comprehensive.csv", col_types = cols_cpt)
cpt_c <- cpt_c %>% 
  filter(boundary == "absolute")

cpt <- cpt %>% mutate(integration = "round")
cpt_c <- cpt_c %>% mutate(integration = "summary")
cpt_both <- bind_rows(cpt, cpt_c)
```
...
gamma_2 <- cpt_both %>%
  filter(parameter == "gamma") %>%
  ggplot(aes(s, mean, color = integration)) +
  facet_wrap(~a, nrow = 2, labeller = labeller(a = as_labeller(label_theta, default = label_parsed)), scales = "free_y") +
  scale_x_continuous(limits = c(0,1), breaks = seq(0,1,.5)) + 
...
ggarrange(gamma_2, delta_2, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right")
```
