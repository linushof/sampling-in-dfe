...
In sum, 100 (parameter combinations), 100 (agents) x 60 (choices) = 600,000 choices are simulated.
...
Table A1 contains the test set of 60 choice problems [Appendix 2][Appendix 2: Choice Problems], which were sampled from an initial set of 10,000.
...
## Plausibility Check: Relationship between Switching Probability and Trial Sample Sizes
...
The median length of trials, i.e., the number of single samples drawn in trial, generated by different parameter combinations ranged from `r min(trial_length$med)` to `r max(trial_length$med)`.
As expected, the scatter plot below shows an inverse relationship between switching probability and trial length.
I.e., the lower the switching probability, the larger become the sample sequences on which each comparison between prospects is based.  
...
The following analyses of choice behavior uses the modeling framework of CPT. 
A non parametric approach, analyzing false response rates, can be found in [Appendix 4][Appendix 4: False Response Rates].
...
cpt_agent %>% 
  ...
  geom_point(data = cpt_agent_mean, aes(color = s), size = 3) +
...
## Appendix 4: False Response Rates 
